...
static void subsys_config_slpi_enable_ramdump(struct subsys_device *dev)
{
	...
	//bool b2 = !!(get_radio_flag() & BIT(3));
	bool b2 = false; /* Disable slpi partial ramdump */
	bool enable = (b0 ^ b1) && b2;
...
static void notif_timeout_handler(unsigned long data)
{
	...
	case HLOS_TO_SUBSYS_SYSMON_SHUTDOWN:
		/* Modem_BSP for "no panic when shutdown modem timeout" ++++ */
		if (NULL != timeout_data->dest_name && !strcmp(timeout_data->dest_name, "modem")) {
			pr_err("shutdown modem subsystem timeout !!! \n");
		} else {
