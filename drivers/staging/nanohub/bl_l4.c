...
#define MAX_BUFFER_SIZE		(1024 * 4) /* for BL2 support */
...
static void stat_bl_log(void)
{
	pr_info("nanohub: MCU flash (Execution time) E:%lldms/%ld/%ld W:%lldms/%ld/%ld R:%lldms/%ld/%ld\n",
		(long long)stat.terase / 1000000, (long)stat.serase,
		(long)stat.cerase,
		(long long)stat.twrite / 1000000, (long)stat.swrite,
		(long)stat.cwrite,
		(long long)stat.tread / 1000000, (long)stat.sread,
		(long)stat.cread);
}
...
static const char *nanohub_status_to_str(uint8_t status)
{
	if (status == CMD_ACK)
		return "CMD_ACK";
	else
...
static uint8_t nanohub_bl_get_version_cmd(struct nanohub_data *data,
					  uint8_t *version)
{
	uint8_t status;
				       ...
				       uint32_t *page)
{
	...
	if ((!psize) || (addr < pdata->flash_banks[0].address) ||
		...
		pr_err("%s: ERR addr is not page-aligned (@0x%08x/@0x%08x),%d)\n",
		       __func__, addr, pdata->flash_banks[0].address,
		       (int)psize);
		return CMD_NACK;
	}
				     ...
				     uint32_t psize)
{
	...
	while ((status == CMD_ACK) && (offset < len)) {
		status = nanohub_bl_write_memory(data, addr, lw,
						 &image[offset]);
		/* next */
		addr   += 256;
		offset += 256;
	...
	while ((status == CMD_ACK) && (offset < length)) {
		...
		if (secn >= 0) {
			...
			if ((status == CMD_ACK) && (diff)) {
				...
				nanohub_bl_get_page_idx(data, addr,
							psize, &page);

				pr_info("nanohub: ERASE/WRITE page %d (@0x%08X/%d, sector:%d diff:%d)\n",
					page, addr, psize, secn, diff);
				nbdiff += diff;
				status = nanohub_bl_erase_page(data,
							       addr, psize);
				if (status == CMD_ACK) {
					status =
					nanohub_bl_write_page(data,
							      addr,
							      &image[offset],
							      lw, psize);
				}
 	...
 	if (status == CMD_ACK) {
		if (nbdiff)
			status = nanohub_bl_toggle_pempty_bit(data);
		else
					     ...
					     uint8_t *ptr, size_t length)
{
	...
	for (i = 0; i < pdata->num_flash_banks; i++) {
		if (addr >= pdata->flash_banks[i].address &&
		    addr < pdata->flash_banks[i].address +
			pdata->flash_banks[i].length) {
			break;
		}
	}
	...
	for (i = 0; status == CMD_ACK && i < pdata->num_flash_banks; i++) {
		...
		if (erase_mask[i]) {
			...
			if (status == CMD_ACK && erase_write_mask[i])
				status = write_bank(data, i, addr, image,
						    length);
...
static uint8_t nanohub_bl_sync_and_check(struct nanohub_data *data)
{
	...
	if ((bl_pid != 0x462) && (pdata->flash_page_size)) {
		pr_err("%s: ERROR - Flash PAGE organization not supported (STM32L45xxx family only)",
		       __func__);
		status = CMD_NACK;
	}
...
static uint8_t nanohub_bl2_load_and_start(struct nanohub_data *data,
					  const uint8_t *image,
					  size_t length)
{
	uint8_t status;
	...
	if ((!image) || (length < sizeof(uint32_t)))
		return CMD_NACK;

	/* As the MCU main stack is placed before the code, MSP value
	 * is equal to the loading @
	 */
	base_laddr = *(uint32_t *)image;

	pr_info("nanohub: BL2 image (@0x%08x,%zd)...\n",
		base_laddr, length);

	status = nanohub_bl_write_memory(data, base_laddr, length, image);

	if (status != CMD_ACK)
		pr_err("%s: nanohub_bl_write_memory() status=%02x\n",
		       __func__, status);
	else {
		status = nanohub_bl_go(data, base_laddr);
		if (status != CMD_ACK)
			pr_err("%s: nanohub_bl_go() status=%02x\n",
			       __func__, status);
	}
...
static uint8_t nanohub_bl_get_cont_region(struct nanohub_data *data,
					  ...
					  uint32_t maxsize)
{
	...
	for (i = 0; i < pdata->num_flash_banks; i++) {
		if (start <= pdata->flash_banks[i].address) {
			start = pdata->flash_banks[i].address;
			end = pdata->flash_banks[i].address +
					pdata->flash_banks[i].length;
			break;
		} else if (start > pdata->flash_banks[i].address &&
				start <
				pdata->flash_banks[i].address +
				pdata->flash_banks[i].length) {
			end = pdata->flash_banks[i].address +
	...
	for (; i < pdata->num_flash_banks; i++) {
		if (end != pdata->flash_banks[i].address)
			break;
		else
			end = pdata->flash_banks[i].address +
			pdata->flash_banks[i].length;
...
uint8_t nanohub_bl2_download(struct nanohub_data *data, uint32_t addr,
			     const uint8_t *image, size_t length,
			     const uint8_t *image2, size_t length2,
			     uint32_t mode)
{
	const struct nanohub_platform_data *pdata = data->pdata;
	uint8_t status;
	struct bl2_info_resp  rbl2info;
	struct bl2_memop_resp rflashinfo;
	uint32_t crc, scrc;
	uint32_t taddr, tsize, off, psize;
	int i;
	ktime_t ts, te;
	uint32_t ms;
	uint32_t nbw, nbe, nbcrc;
	...
	pr_info("nanohub: Upgrading the full image (@0x%08x,%zd) BL2(mode=%d)...\n",
		addr, length, mode);
	...
	ts = ktime_get();
	status = nanohub_bl2_load_and_start(data, image2, length2);
	te = ktime_get();
	pr_info("nanohub: Loading and starting BL2 image %s (%lldms)\n",
		nanohub_status_to_str(status),
		ktime_to_ms(ktime_sub(te, ts)));
	if (status != CMD_ACK)
		goto out;

	/* Delay to wait the end of the MCU boot */
	msleep(100);

	i = 10;
	do {
		status = nanohub_bl2_sync(data, MAX_BUFFER_SIZE);
		/* Delay to be sure that the MCU is ready to receive a first
		 * command. This is to be safe, normally it is not necessary
		 * with the fix #475879 in MCU firmware.
		 */
		usleep_range(1000, 1200);
		if (status == CMD_ACK)
			status = nanohub_bl2_info(data, &rbl2info);
		i--;
	} while ((status != CMD_ACK) && (i));

	if (status != CMD_ACK) {
		pr_err("%s: Sync and read info from BL2 has failed\n",
		       __func__);
		goto out;
	}

	pr_info("nanohub: BL2 protocol v%d.%d(%d.%d) DevID:0x%04x (maxIOData=%d)\n",
		BL2_INFO_API_MAJOR(rbl2info.ver),
		BL2_INFO_API_MINOR(rbl2info.ver),
		BL2_INFO_IMP_MAJOR(rbl2info.ver),
		BL2_INFO_IMP_MINOR(rbl2info.ver),
		rbl2info.id & 0xFFFF,
		rbl2info.max_size);

	if (BL2_INFO_API_MAJOR(rbl2info.ver) != BL2_API_VERSION_MAJOR) {
		status = CMD_NACK;
		pr_err("%s: BL2 API is not supported (expected %d.x)\n",
		       __func__, BL2_API_VERSION_MAJOR);
		goto out;
	}

	status = nanohub_bl2_flashinfo(data, ~0, &rflashinfo);
	if (status != CMD_ACK)
		goto out;

	psize = rflashinfo.size / BL2_FLASH_GET_SECTOR_COUNT(rflashinfo.extra);

	pr_info("nanohub: MCU flash desc. @0x%08x/%dKB %sUNIFIED PEMPTY=%db (#%d/%dKB,pu=%d)\n",
		rflashinfo.address,
		(int)rflashinfo.size / 1024,
		BL2_FLASH_IS_UNIFIED(rflashinfo.extra) ? "" : "NON-",
		BL2_FLASH_PEMPTY_BIT(rflashinfo.extra),
		(int)BL2_FLASH_GET_SECTOR_COUNT(rflashinfo.extra),
		(int)psize / 1024,
		(int)BL2_FLASH_GET_PUINT(rflashinfo.extra));

	/* Checking if the image can be flashed */
	if ((addr > rflashinfo.address) || (rflashinfo.size < length) ||
	    (addr % BL2_FLASH_GET_PUINT(rflashinfo.extra)) ||
	    (psize != pdata->flash_page_size)) {
		status = CMD_NACK;
		pr_err("%s: @/size/psize are not valid\n", __func__);
		goto out;
	}

	ts = ktime_get();

	/* Erase ALL mode is forced to be sure to have a complete
	 * blank main flash
	 */
	if (BL2_FLASH_PEMPTY_BIT(rflashinfo.extra))
		mode = 2;

	if (mode == 2) {
		status = nanohub_bl2_flasherase(data, ~0, 0, &ms);
		pr_info("nanohub: Erasing ALL (%dms) %s\n",
			ms, nanohub_status_to_str(status));
		nbe += rflashinfo.size;
		if (status != CMD_ACK)
			goto out;
	}

	/* Main loop to check and to program */
	...
	while ((off < length) && (status == CMD_ACK) &&
	       (i < BL2_FLASH_GET_SECTOR_COUNT(rflashinfo.extra))) {
		uint32_t pg0, pgN;

		nanohub_bl_get_cont_region(data, &taddr, &tsize,
					   (mode == 0) ? psize : 0);
		if (tsize == 0) {
			status = CMD_NACK;
			pr_err("%s: Outside the logical sections 0x%08x\n",
			       __func__, taddr);
			goto out;
		}

		off = taddr - addr;
		tsize = ((length - off) > tsize) ? tsize : (length - off);

		pg0 = (taddr - rflashinfo.address) / psize;
		pgN = tsize / psize;
		if ((tsize % psize) || (tsize < psize))
			pgN += 1;

		crc = BL2_CRC32_INIT_DEFAULT;
		scrc = BL2_CRC32_INIT_DEFAULT;

		if ((mode == 0) || (mode == 1)) {
			if (debug)
				pr_info("nanohub: [%d] Checking section 0x%08x/%d (pg=%d..%d,off=%d)..\n",
					i, taddr, tsize, pg0, pg0 + pgN - 1,
					off);

			status = nanohub_bl2_flashcrc32(data, taddr, tsize,
							&crc, &ms);
			if (debug)
				pr_info("nanohub: [%d] CRC32 calculation (%dms) %s\n",
					i, ms, nanohub_status_to_str(status));

			nbcrc += tsize;
			if (status != CMD_ACK)
				goto out;

			scrc = crc32(image + off, tsize, scrc);

			if (debug)
				pr_info("nanohub: [%d] crc32(0x%08x/%d)=0x%08x/0x%08x (%s)\n",
					i, taddr, tsize, crc, scrc,
					(crc != scrc) ? "Different" :
							"Identical");

			/* Erase if diff */
			if (scrc != crc) {
				status = nanohub_bl2_flasherase(data, pg0, pgN,
								&ms);

				if (debug)
					pr_info("nanohub: [%d] Erasing pages (%dms) %s\n",
						i, ms,
						nanohub_status_to_str(status));
		...
		if ((mode == 2) || (scrc != crc)) {
			status = nanohub_bl2_flashprog(data, taddr,
						       image + off, tsize, &ms);
			if (!ms)
				ms = 1;
			if (debug)
				pr_info("nanohub: [%d] Writing section 0x%08x/%d (%dms %dKiB/s) %s\n",
					i, taddr, tsize, ms,
					(tsize * 1024) / (ms * 1000),
					nanohub_status_to_str(status));
			nbw += tsize;
...
out:
	ms = ktime_to_ms(ktime_sub(te, ts));
	if (!ms)
		ms = 1;

	pr_info("nanohub: %s UPDATE C:%d E:%d W:%d (mode=%d, iter=%d, %dms %dKiB/s)\n",
		(nbe == 0) ? "NO" : (mode != 2) ? "PARTIAL" : "FULL",
		nbcrc, nbe, nbw,
		mode, i, ms, (nbw * 1024) / (ms * 1000));
			    ...
			    const uint8_t *image, size_t length)
{
	...
	if (status != CMD_ACK)
		pr_err("%s: nanohub_bl_read_memory() status=0x%02x\n",
			...
			if (pdata->flash_page_size)
				status =
				nanohub_bl_download_by_page(data, addr, image,
							    ptr, length);
			else
				status =
				nanohub_bl_download_by_sector(data, addr, image,
							      ptr, length);
	}
	do_gettimeofday(&te);
...
uint8_t nanohub_bl_erase_shared_by_page(struct nanohub_data *data)
{
	...
	status = CMD_ACK;
	for (i = 0; (status == CMD_ACK) && (i < pdata->num_shared_flash_banks);
		i++) {
...
uint8_t nanohub_bl_erase_shared(struct nanohub_data *data)
{
	...
	if (pdata->flash_page_size) {
		...
		for (i = 0; status == CMD_ACK &&
		     i < pdata->num_shared_flash_banks; i++) {
