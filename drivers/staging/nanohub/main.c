...
static bool mfgmode = false;
static bool drop_buf_change_flag = false;	//a switch to control if drop_buf flag can be set by other drivers
...
#define EVT_HTC_BASE            0x00001000
#define APP_BYPASS_HOST_EVENTID        (EVT_HTC_BASE + 2)
...
static uint8_t EDGE_KEY_ID[TOTAL_HW_STAGE] = {EVT2, BEFORE_EVT2, EVT3, EVT4, EVT5, AFTER_EVT5};
...
static struct nanohub_buf *nanohub_io_get_buf(struct nanohub_io *io, bool wait);
static void nanohub_io_put_buf(struct nanohub_io *io, struct nanohub_buf *buf);
...
static int nanohub_comms_app_from_host(struct nanohub_data *data,
		uint64_t appId, const uint8_t *buffer, size_t buffer_len);
#ifdef CONFIG_NANOHUB_HTC_LOG
					 ...
					 unsigned long event, void *data)
{
	...
	struct nanohub_data *nanohub_data =
		container_of(self, struct nanohub_data, drm_notifier);
...
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
				cancel_delayed_work(&nanohub_data->work_temperature);
				nanohub_get_temperature();
				nanohub_data->edge_ultra_cfg.display_status = false;
				nanohub_comms_write_cfg_data(nanohub_data, SENS_TYPE_HTC_EDGE_ULTRA,
						(uint8_t *)&nanohub_data->edge_ultra_cfg, sizeof(struct edge_ultra_cfg_data));
#endif
			}
...
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
				cancel_delayed_work(&nanohub_data->work_temperature);
				queue_delayed_work(nanohub_data->wq, &nanohub_data->work_temperature, 0);
#endif
			}
			nanohub_get_log(nanohub_data);
			break;
...
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
	data->edge_ultra_cfg.display_status = (status & NANOHUB_TP_SWITCH_MCU_NORMAL) ? false : true;
...
static void nanohub_vbus_wq(struct work_struct *work)
{
	int ret = 0;
	struct nanohub_data *data = s_data;
	pr_info("nanohub: [EGR] vbus_state = %d, key_state = %d, edge_state = %d\n", data->edge_cfg.usb_status, data->edge_cfg.key_status, data->edge_cfg.edge_status);
...
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
	if (data->edge_ultra_cfg.power_key_status) {
		ret = nanohub_comms_write_cfg_data(data, SENS_TYPE_HTC_EDGE_ULTRA,
				(uint8_t *)&data->edge_ultra_cfg, sizeof(struct edge_ultra_cfg_data));
		if(!ret)
			data->edge_ultra_cfg.power_key_status = 0;
	}
#endif
}
...
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
		//track the usb status only
		data->edge_ultra_cfg.usb_status = status_in_out;
#endif
...
static int nanohub_get_temperature(void)
{
	...
	if(data == NULL) {
		pr_info("nanohub_get_temp: data not ready.\n");
		return -1;
	}

	tzd = thermal_zone_get_zone_by_name("quiet-therm-adc");
	if(IS_ERR(tzd) || tzd == NULL) {
		pr_info("nanohub_get_temp: get_zone failed.\n");
		return -1;
	}
	ret = tzd->ops->get_temp(tzd, &temperature);
	if(ret) {
		pr_info("nanohub_get_temp: get_temp failed.\n");
		return -1;
	}
	pr_info("nanohub_get_temp: temperature = %d\n", temperature);
	data->edge_ultra_cfg.temperature = temperature;

	return 0;
}

static void nanohub_temperature_wq(struct work_struct *work)
{
	struct nanohub_data *data =
			container_of(to_delayed_work(work), struct nanohub_data, work_temperature);
	int ret = 0;

	ret = nanohub_get_temperature();
	if(ret) {
		queue_delayed_work(data->wq, &data->work_temperature, msecs_to_jiffies(10000));
	} else {
		nanohub_comms_write_cfg_data(data, SENS_TYPE_HTC_EDGE_ULTRA,
				(uint8_t *)&data->edge_ultra_cfg, sizeof(struct edge_ultra_cfg_data));
		queue_delayed_work(data->wq, &data->work_temperature, msecs_to_jiffies(30000));
	}
	return;
}
...
int nanohub_edge_vol_wakeup(uint8_t bitmask)
{
	...
	if (data->edge_ultra_cfg.vol_key_can_wake != bitmask) {
		data->edge_ultra_cfg.vol_key_can_wake = bitmask;
		nanohub_comms_write_cfg_data(data, SENS_TYPE_HTC_EDGE_ULTRA,
				(uint8_t *)&data->edge_ultra_cfg, sizeof(struct edge_ultra_cfg_data));
	} else {
		pr_info("%s: skip the same value\n", __func__);
	}
	return 0;
}
...
int nanohub_edge_debug(bool enable) {
	...
	if (data->edge_ultra_cfg.debug != enable) {
		data->edge_ultra_cfg.debug = enable;
		nanohub_comms_write_cfg_data(data, SENS_TYPE_HTC_EDGE_ULTRA,
				(uint8_t *)&data->edge_ultra_cfg, sizeof(struct edge_ultra_cfg_data));
	} else {
		pr_info("%s: skip the same value\n", __func__);
...
int nanohub_edge_dynamic_power_ctrl(bool enable) {
	...
	if (data->edge_ultra_cfg.dynamic_power_ctrl != enable) {
		data->edge_ultra_cfg.dynamic_power_ctrl = enable;
		nanohub_comms_write_cfg_data(data, SENS_TYPE_HTC_EDGE_ULTRA,
				(uint8_t *)&data->edge_ultra_cfg, sizeof(struct edge_ultra_cfg_data));
	} else {
		pr_info("%s: skip the same value\n", __func__);
...
void nanohub_edge_status(uint8_t status)
{
	...
	if(EDGE_KEY_ID[status] != data->edge_cfg.edge_status) {
		data->edge_cfg.edge_status = EDGE_KEY_ID[status];
		queue_work(data->wq, &data->work_vbus);
	}
...
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
int nanohub_key_status(unsigned int code, uint8_t status)
{
	struct nanohub_data *data = s_data;
	...
	if (status) {
		...
		if (code == KEY_POWER)
			data->edge_ultra_cfg.power_key_status = status;
...
static void nanohub_reset_event_handler(struct nanohub_data *data)
{
	...
	if ((pdata->motion_sensor_placement != 0xFF)
		| (pdata->mag_pdc_matrix_flag != 0)) {
		pr_info("nanohub: Restore placement = 0x%x, flag = 0x%x\n",
			pdata->motion_sensor_placement, pdata->mag_pdc_matrix_flag);
		buf[0] = pdata->motion_sensor_placement;
		buf[1] = pdata->mag_pdc_matrix_flag;
		buf[2] = 0;
		nanohub_comms_app_from_host(data, BMI160_APP_ID, buf, 3);
	}
}

static void nanohub_resume_wq(struct work_struct *work)
{
	struct nanohub_data *data =
			container_of(work, struct nanohub_data, work_resume);
...
#endif
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
		if (data->cfg_restore_type[i] == SENS_TYPE_HTC_EDGE_ULTRA) {
			nanohub_comms_write_cfg_data(data, SENS_TYPE_HTC_EDGE_ULTRA,
				(uint8_t *)&data->edge_ultra_cfg, sizeof(struct edge_ultra_cfg_data));
			pr_info("nanohub: restore sens_type=%d\n", data->cfg_restore_type[i]);
			data->cfg_restore_type[i] = 0;
		...
		uint64_t appId, const uint8_t *buffer, size_t buffer_len)
{
	...
	tx_len = sizeof(struct MsgCmd) + buffer_len;
	cmd = (struct HostCmd *)vmalloc(tx_len);
	if (!cmd) {
		return -ENOMEM;
	}

	cmd->command.evtType = EVT_APP_FROM_HOST;
	cmd->command.msg.appId = appId;
	cmd->command.msg.dataLen = sizeof(uint8_t) * buffer_len;
	memcpy(cmd->data, buffer, buffer_len);

	lock_mode = atomic_read(&data->lock_mode);
	if (lock_mode != LOCK_MODE_NONE) {
		pr_info("nanohub: app_from_host skipped, lock_mode=%d, appId=%llu\n",
			lock_mode, appId);
		vfree(cmd);
		return -EAGAIN;
	}
	...
	if (ret) {
		pr_err("nanohub: app_from_host failed, ret=%d, appId=%llu, buffer_len=%lu\n",
			ret, appId, buffer_len);
	}
...
static void nanohub_bypass_to_app(uint64_t appId, uint8_t *buf, uint8_t len)
{
	struct nanohub_data *data = s_data;

	if (data == NULL) {
		pr_warn("%s: not probe yet\n", __func__);
		return;
	}

	nanohub_comms_app_from_host(data, appId, buf, len);
}

static void nanohub_bypass_notifier_register(struct nanohub_bypass_notifier *notifier)
{
	struct nanohub_data *data = s_data;
	...
	data->bypass_notifier = notifier;
}

static void nanohub_bypass_buf(struct nanohub_data *data, uint8_t *buffer)
{
	struct MsgCmd *cmd_buf = (struct MsgCmd *)buffer;

	if (data->bypass_notifier == NULL) {
		return;
	}

	if (data->bypass_notifier->appId == cmd_buf->msg.appId) {
		if (data->bypass_notifier->callback)
			data->bypass_notifier->callback(buffer+(sizeof(struct MsgCmd)), cmd_buf->msg.dataLen);
	}
}

void nanohub_edge_bypass_to_app(uint8_t *buf, uint8_t len)
{
	uint64_t appId = SNT_APP_ID;

	nanohub_bypass_to_app(appId, buf, len);
}
EXPORT_SYMBOL(nanohub_edge_bypass_to_app);

void nanohub_edge_notifier_register(struct nanohub_bypass_notifier *notifier)
{
	notifier->appId = SNT_APP_ID;
	nanohub_bypass_notifier_register(notifier);
}
EXPORT_SYMBOL(nanohub_edge_notifier_register);
#endif
...
static int nanohub_get_log(struct nanohub_data *data)
{
	...
	if (nanohub_comms_tx_rx_retrans
		...
		getnstimeofday(&ts_t);
		rtc_time_to_tm(ts_t.tv_sec, &tm);
		pr_info("nanohub: %s: time: %ld.%03ld (%d-%02d-%02d %02d:%02d:%02d.%03lu UTC)\n",
			__func__, ts.tv_sec, ts.tv_nsec/1000000,
			tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
			tm.tm_hour, tm.tm_min, tm.tm_sec, ts_t.tv_nsec/1000000);
	} else {
...
static void nanohub_edge_monitor_control(struct nanohub_data *data,
	uint8_t is_read, uint8_t reg, uint8_t val)
{
	pr_info("%s: %c: %x: %x\n", __func__, is_read?'R':'W', reg, val);

	data->egm_cfg.isRead = is_read;
	data->egm_cfg.reg = reg;
	data->egm_cfg.value = val;

	nanohub_comms_write_cfg_data(data, SENS_TYPE_HTC_EG_MONITOR,
		(uint8_t *)&data->egm_cfg, sizeof(struct egm_cfg_data));
}

static ssize_t edge_monitor_control(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct nanohub_data *data = dev_get_drvdata(dev);
	int ret;
	uint32_t val;
	uint8_t is_read, reg, value;
	...
	pr_info("%s: val:%x\n", __func__, val);
	is_read = (uint8_t)((val >> 16) & 0xFF);
	reg = (uint8_t)((val >> 8) & 0xFF);
	value = (uint8_t)(val & 0xFF);

	nanohub_edge_monitor_control(data, is_read, reg, value);

	return count;
}
#endif

static ssize_t get_drop_buf_flag(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct nanohub_data *data = dev_get_drvdata(dev);

	return scnprintf(buf, PAGE_SIZE, "drop_buf_flag = %u\n", data->drop_buf_flag);
}

static ssize_t set_drop_buf_flag(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct nanohub_data *data = dev_get_drvdata(dev);
	struct nanohub_io *io = &data->io[ID_NANOHUB_SENSOR];
	struct nanohub_buf *temp_buf;
	int ret;
	uint32_t val;
	...
	pr_info("nanohub: drop_buf_flag = %u\n", val);

	data->drop_buf_flag = val;

	while (nanohub_io_has_buf(io)) {
		temp_buf = nanohub_io_get_buf(io, false);
		nanohub_io_put_buf(&data->free_pool, temp_buf);
	}
...
int nanohub_set_drop_buf_flag(char *buf, size_t count)
{
	...
	if (drop_buf_change_flag) {
		if (htc_sensorhub_dev != NULL)
			set_drop_buf_flag(htc_sensorhub_dev, NULL, buf, count);
		else
			return -ENODEV;
	}
...
EXPORT_SYMBOL(nanohub_set_drop_buf_flag);
#endif

#ifdef CONFIG_NANOHUB_EDGE_ULTRA
static ssize_t get_quick_launch_cam(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct nanohub_data *data = dev_get_drvdata(dev);

	return scnprintf(buf, PAGE_SIZE, "quick_launch_cam = %u\n", data->edge_ultra_cfg.quick_launch_cam);
}

static ssize_t set_quick_launch_cam(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct nanohub_data *data = dev_get_drvdata(dev);
	int ret;
	...
	pr_info("nanohub: quick_launch_cam = %u\n", val);

	data->edge_ultra_cfg.quick_launch_cam = val;
...
static ssize_t set_cancel_force_reset(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct nanohub_data *data = dev_get_drvdata(dev);
	int ret;
	...
	pr_info("nanohub: cancel_force_reset = %u\n", val);

	if (data->edge_ultra_cfg.normal_mode && val == 1) {
		data->edge_ultra_cfg.cancel_force_reset = true;

		nanohub_comms_write_cfg_data(data, SENS_TYPE_HTC_EDGE_ULTRA,
				(uint8_t *)&data->edge_ultra_cfg, sizeof(struct edge_ultra_cfg_data));
		data->edge_ultra_cfg.cancel_force_reset = false;
	} else if (!data->edge_ultra_cfg.normal_mode && val == 2) {
		data->edge_ultra_cfg.normal_mode = true;
		nanohub_comms_write_cfg_data(data, SENS_TYPE_HTC_EDGE_ULTRA,
				(uint8_t *)&data->edge_ultra_cfg, sizeof(struct edge_ultra_cfg_data));
	}
...
#endif
#ifdef CONFIG_NANOHUB_EDGE_MONITOR_CONTROL
	__ATTR(edge_monitor_control, 0220, NULL, edge_monitor_control),
#endif
	__ATTR(drop_buf, 0660, get_drop_buf_flag, set_drop_buf_flag),
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
	__ATTR(qlaunch_cam_setting, 0660, get_quick_launch_cam, set_quick_launch_cam),
	__ATTR(cancel_force_reset, 0220, NULL, set_cancel_force_reset),
#endif
};
/* HTC_END */
...
static ssize_t nanohub_download_bl2(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	struct nanohub_data *data = dev_get_nanohub_data(dev);
	const struct nanohub_platform_data *pdata = data->pdata;
	const struct firmware *fw_entry;
	const struct firmware *bl2_entry;
	int ret;
	uint8_t status = CMD_ACK;
#ifdef CONFIG_NANOHUB_FLASH_RETRY
	int retry = 5;
	...
	uint32_t mode = 0; /* Default mode, contents of the MCU flash is verified
	                      sector/page granularity */

	/* Mode 1 indicates that the flash is verified by logical continuous
	 *  code section (in our case: BL & OS sections)
	 * Mode 2 indicates that the flash should be fully erased before
	 *  the update (force mode)
	 *
	 * \note: If an program empty flash is detected, a full erase algorithm
	 *        is applied.
	 *        mode = 1x allows to set the debug mode.
	 */
	if (count > 0) {
			if (buf[0] == '2')
				mode = 2;
	...
	nanohub_flash_status = 1;
#endif
	/* Try to request the bl2 firmware/image */
	ret = request_firmware(&bl2_entry, "nanohub.bl2.bin", dev);
	if (ret) {
		dev_err(dev, "%s: err=%d\n", __func__, ret);
		return ret;
	}
...
flash_start:
	...
	ret = request_firmware(&fw_entry, "nanohub.full.bin", dev);
	if (ret) {
#ifndef CONFIG_NANOHUB_FLASH_RETRY
		release_firmware(bl2_entry);
#endif
		dev_err(dev, "%s: err=%d\n", __func__, ret);
	} else {
		status = nanohub_bl2_download(data, pdata->bl_addr,
					     fw_entry->data, fw_entry->size,
						 bl2_entry->data, bl2_entry->size,
						 mode);
		dev_info(dev, "%s: status=%02x\n", __func__, status);
		release_firmware(fw_entry);
#ifndef CONFIG_NANOHUB_FLASH_RETRY
...
#ifdef CONFIG_NANOHUB_STM32L4
	__ATTR(download_bl2, 0220, NULL, nanohub_download_bl2),
#endif
...
/* HTC_END */
...
#ifdef CONFIG_NANOHUB_BYPASS_MODE
	if (event_id == APP_BYPASS_HOST_EVENTID) {
		nanohub_bypass_buf(data, (*buf)->buffer);
	}
#endif
	...
	*buf = NULL;
	...
	if (wakeup)
		__pm_wakeup_event(&data->wakesrc_read, msecs_to_jiffies(250));
	release_wakeup(data);
...
/* HTC_START */
	...
	ret = of_property_read_u8(dt, "sensorhub,mag-pdc-matrix-flag",
				&pdata->mag_pdc_matrix_flag);
	if (ret < 0) {
		pdata->mag_pdc_matrix_flag = 0;
		pr_info("nanohub: NO PDC matrix flag in DT\n");
	} else {
		pr_info("nanohub: PDC matrix flag = 0x%x\n", pdata->mag_pdc_matrix_flag);
	}
/* HTC_END */
...
struct iio_dev *nanohub_probe(struct device *dev, struct iio_dev *iio_dev)
{
	...
	atomic_set(&data->kthread_run, 0);
	wakeup_source_init(&data->wakesrc_read, "nanohub_wakesrc_read");
...
/* HTC_START */
	...
	INIT_WORK(&data->work_resume, nanohub_resume_wq);
#ifdef CONFIG_NANOHUB_EDGE
	INIT_WORK(&data->work_vbus, nanohub_vbus_wq);
#endif
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
	INIT_DELAYED_WORK(&data->work_temperature, nanohub_temperature_wq);
#endif
#ifdef CONFIG_NANOHUB_FLASH_GPIO_CONTROL
	...
	data->edge_cfg.edge_status = AFTER_EVT5;
	...
	if ((strcmp(htc_get_bootmode(), "download") == 0)
		...
		drop_buf_change_flag = true;
		pr_info("nanohub: drop_buf_change_flag=1\n");
	}

	data->edge_ultra_cfg.threshold		= 0x00;
	data->edge_ultra_cfg.vol_key_can_wake	= 0x00;
	data->edge_ultra_cfg.quick_launch_cam	= 0;

	data->edge_ultra_cfg.dynamic_power_ctrl	= true;
	if ((mfgmode == true) || (strcmp(htc_get_bootmode(), "ftm") == 0)) {
		data->edge_ultra_cfg.dynamic_power_ctrl	= false;
	}

	data->edge_ultra_cfg.offmode_charging	= false;
	if (strcmp(htc_get_bootmode(), "offmode_charging") == 0) {
		data->edge_ultra_cfg.offmode_charging	= true;
	}

	if (tamper_flag == 0) {
		data->edge_ultra_cfg.debug		= true;
	} else
		data->edge_ultra_cfg.debug		= false;

	data->edge_ultra_cfg.normal_mode	= false;
	data->edge_ultra_cfg.cancel_force_reset	= false;
	data->edge_ultra_cfg.display_status	= true;
	data->edge_ultra_cfg.reserved		= false;
	data->edge_ultra_cfg.usb_status		= 255;

	if (data->edge_ultra_cfg.offmode_charging
			|| (data->edge_ultra_cfg.dynamic_power_ctrl == false)) {
		pr_info("nanohub: bootmode: %s%s\n", htc_get_bootmode(), mfgmode ? ", mfg" : "");
	}
#endif
#endif
/* HTC_END */
...
#ifdef CONFIG_NANOHUB_HTC_LOG
#ifdef CONFIG_DRM
	data->drm_notifier.notifier_call = drm_notifier_callback;
	msm_drm_register_client(&data->drm_notifier);
#endif
#ifdef CONFIG_FB
...
/* HTC_START */
	queue_work(data->wq, &data->work_resume);
/* HTC_END */
...
#ifdef CONFIG_NANOHUB_EDGE
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
static int __init nanohub_mfgmode(char *str)
{
	pr_info("nanohub: androidboot.mfg.edgeconfig: %s\n", str);
	...
	return 0;
} early_param("androidboot.mfg.edgeconfig", nanohub_mfgmode);

static int __init get_tamper_flag(char *str)
{
	int ret = kstrtouint(str, 0, &tamper_flag);
	pr_info("nanohub: tamper_flag: %d, ret : %d", tamper_flag, ret);
	return ret;
