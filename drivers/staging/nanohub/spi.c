...
static int spi_bl_txrx(const void *data, int length)
{
...
static void nanohub_do_reset(struct work_struct *work)
{
	struct nanohub_spi_data *spi_data = container_of((struct work_struct *)work,
                        struct nanohub_spi_data, reset_work);
	pr_info("%s++:\n", __func__);
...
static int nanohub_spi_probe(struct spi_device *spi)
{
	...
	/* HTC ++ */
	spi_data->wq = create_singlethread_workqueue("nanohub_spi_wq");
	if (spi_data->wq == NULL) {
		pr_err("Not able to create workqueue\n");
		nanohub_reset(&spi_data->data);
	} else {
		INIT_WORK(&spi_data->reset_work, nanohub_do_reset);
		queue_work(spi_data->wq, &spi_data->reset_work);
	}
	/* HTC -- */

	/*nanohub_reset(&spi_data->data);*/ /* Move this to another work to speed up boot process */
