...
{
	int pulldown = 0, input_en = 0, output = 0;
...
{
	struct tca6418_data *pdata = NULL;
	int ret = 0;
	int io_exp_rst_gpio = 0;
	...
	io_exp_rst_gpio = of_get_named_gpio(pdev->dev.of_node, "io_exp_rst", 0);
	if (gpio_is_valid(io_exp_rst_gpio)) {
		ret = gpio_request(io_exp_rst_gpio, "IO_EXT_RST#");
		if (ret) {
			dev_err(&pdev->dev, "%s: Failed to request gpio %d\n",__func__, io_exp_rst_gpio);
			goto fail_gpio_request;
		}
		gpio_direction_output(io_exp_rst_gpio, 1);
		msleep(1);
		dev_info(&pdev->dev, "%s: io_exp_rst_gpio = %d\n",__func__, gpio_get_value(io_exp_rst_gpio));
	}
