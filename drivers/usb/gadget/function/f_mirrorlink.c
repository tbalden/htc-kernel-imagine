...
extern int dsi_get_display_max_size(void);
...
{
	char *ml_disable[3] = { "SWITCH_NAME=mirror_link", "SWITCH_STATE=0", NULL };

	kobject_uevent_env(&ml_switch.this_device->kobj, KOBJ_CHANGE, ml_disable);
	return;
}
...
static void mirrorlink_notify_display(struct work_struct *w)
{
	...
	char *ml_status_offline[3] = { "SWITCH_NAME=ml_status", "SWITCH_STATE=offline", NULL };
	char *ml_status_online[3] = { "SWITCH_NAME=ml_status", "SWITCH_STATE=online", NULL };
	char *ml_status_projecting[3] = { "SWITCH_NAME=ml_status", "SWITCH_STATE=projecting", NULL };
	pr_debug("%s\n", __func__);

	kobject_uevent_env(&dev->ml_status_sdev.this_device->kobj, KOBJ_CHANGE,
		(atomic_read(&dev->ml_status) == ML_PROJECTING ?
		ml_status_projecting : (atomic_read(&dev->ml_status) == ML_ONLINE ?
		ml_status_online : ml_status_offline)) );
...
static void mirrorlink_notify_setting(struct work_struct *w)
{
	...
	char *ml_disable[3] = { "SWITCH_NAME=mirror_link", "SWITCH_STATE=0", NULL };
	char *ml_enable[3] = { "SWITCH_NAME=mirror_link", "SWITCH_STATE=1", NULL };
	pr_debug("%s\n", __func__);

	kobject_uevent_env(&ml_switch.this_device->kobj, KOBJ_CHANGE,
		atomic_read(&dev->ml_enable_HSML) ? ml_enable : ml_disable );
...
//	dev->start_send_fb = false;
	...
	if (atomic_read(&dev->ml_enable_HSML) != 0) {
		...
		pr_info("[mirrorlink]%s, set state: 0\n",__func__);
		...
		struct usb_function *f)
{
	...
	pr_debug("[mirrorlink] resolution: %u*%u"
...
static void mirrorlink_complete_req(struct usb_ep *ep, struct usb_request *req)
{
	...
	pr_debug("[mirrorlink] status=%d, request=0x%02X\n",
											...
											const struct usb_ctrlrequest *ctrl)
{
	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_VENDOR) {
		pr_debug("[mirrorlink] request(req=0x%02x, wValue=%d, "
						"wIndex=%d, wLength=%d)\n",
		...
		switch (ctrl->bRequest) {
			case HSML_08_REQ_MIRROR_LINK:
...
#else
					pr_info("[mirrorlink]%s, set state: 1\n", __func__);
#endif
					atomic_set(&_ml_dev->ml_enable_HSML, 1);
				...
				break;
			default:
				pr_debug("[mirrorlink] unrecognized request(req=0x%02x, wValue=%d, "
						"wIndex=%d, wLength=%d)\n",
						ctrl->bRequest, ctrl->wValue, ctrl->wIndex, ctrl->wLength);
				break;
		}
	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_VENDOR) {
		...
		if (_ml_dev->hsml_ver == cHSML_VER_12) {
			switch (ctrl->bRequest) {
				...
				default:
					pr_debug("[mirrorlink] unrecognized request (HSML_12) (req=0x%02x, wValue=%d, "
							"wIndex=%d, wLength=%d)\n",
							ctrl->bRequest, ctrl->wValue, ctrl->wIndex, ctrl->wLength);
					break;
			}
...
#endif
		{
			switch (ctrl->bRequest) {
				...
				case HSML_08_REQ_GET_SERVER_DISPLAY: {
					int maxSize = 0;
#ifdef CONFIG_DRM
					maxSize = dsi_get_display_max_size();
#else
					struct fb_info *info;
					...
					pr_debug("[mirrorlink] res_supported=0x%x, pixel_format_supported=0x%x, w_length=%d\n",
							*(unsigned int*)&(_ml_dev->hsml_proto->get_display_capabilities_info.dwResolutionSupported),
							*(unsigned int*)&(_ml_dev->hsml_proto->get_display_capabilities_info.dwPixelFormatSupported),
							w_length);
				...
				default:
					pr_debug("[mirrorlink] unrecognized request (HSML_08) (req=0x%02x, wValue=%d, "
							"wIndex=%d, wLength=%d)\n",
							ctrl->bRequest, ctrl->wValue, ctrl->wIndex, ctrl->wLength);
					break;
			}
...
#if 0//old style using switch_dev
static ssize_t print_ml_switch_name(struct switch_dev *sdev, char *buf)
{
	return sprintf(buf, "%s\n", ml_shortname);
}

static ssize_t print_ml_switch_state(struct miscdevice *ml_status_sdev,
		char *buf)
{
...
static int mirrorlink_setup(struct hsml_protocol *config)
{
	...
	ret = misc_register(&ml_switch);
	if (ret < 0) {
		pr_err("fail to register mirror_link misc!\n");
		goto err_free;
	}

	dev->ml_status_sdev.minor = MISC_DYNAMIC_MINOR;
...
#endif
	ret = misc_register(&dev->ml_status_sdev);
	if (ret < 0) {
		pr_err("usb ml_status_sdev misc_deregister register fail\n");
		goto err_free;
	}
