...
static inline struct f_charger_opts *ci_to_f_charger_opts(struct config_item *item)
{
	return container_of(to_config_group(item), struct f_charger_opts,
			func_inst.group);
}

static inline struct f_charger_opts *fi_to_f_charger_opts(struct usb_function_instance *fi)
{
	return container_of(fi, struct f_charger_opts, func_inst);
}

static int charger_set_inst_name(struct usb_function_instance *fi,
		const char *name)
{
	struct f_charger_opts *opts = fi_to_f_charger_opts(fi);
	char *ptr;
	int name_len;
...
static void charger_free_instance(struct usb_function_instance *fi)
{
	struct f_charger_opts *opts = fi_to_f_charger_opts(fi);
...
static void charger_opts_release(struct config_item *item)
{
	struct f_charger_opts *opts = ci_to_f_charger_opts(item);

	usb_put_function_instance(&opts->func_inst);
}

static struct configfs_item_operations charger_item_ops = {
	.release        = charger_opts_release,
};

/*static struct configfs_attribute *charger_attrs[] = {
	NULL,
};*/

static struct config_item_type charger_func_type = {
	.ct_item_ops    = &charger_item_ops,
	//.ct_attrs       = charger_attrs,
...
static int charger_setup(struct usb_function *f,
...
static void charger_free_func(struct usb_function *f)
{
	struct f_charger *charger = func_to_charger(f);
...
static struct usb_function_instance *charger_alloc_inst(void)
{
	struct f_charger_opts *opts;

	opts = kzalloc(sizeof(*opts), GFP_KERNEL);
	if (!opts)
		return ERR_PTR(-ENOMEM);

	opts->func_inst.set_inst_name = charger_set_inst_name;
	opts->func_inst.free_func_inst = charger_free_instance;
...
static struct usb_function *charger_alloc(struct usb_function_instance *fi)
{
	...
	charger->func.free_func = charger_free_func;
	charger->func.set_alt = charger_set_alt;
	charger->func.disable = charger_disable;
	charger->func.setup   = charger_setup;

	return &charger->func;
}

DECLARE_USB_FUNCTION_INIT(charger, charger_alloc_inst, charger_alloc);
MODULE_LICENSE("GPL");
