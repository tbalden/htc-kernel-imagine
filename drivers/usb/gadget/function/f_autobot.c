...
static void cand_online_notify(struct work_struct *w)
{
	...
	snprintf(name_buf, sizeof(name_buf), "SWITCH_NAME=%s", cand_shortname);
	snprintf(state_buf, sizeof(state_buf), "SWITCH_STATE=%s",
		(atomic_read(&dev->cand_online) ? "online" : "offline"));

	envp[0] = name_buf;
	envp[1] = state_buf;
	envp[2] = NULL;

	kobject_uevent_env(&dev->cand_sdev.this_device->kobj, KOBJ_CHANGE, envp);
}
...
static void htcmode_status_notify(struct work_struct *w)
{
	...
	if (dev->htcmode_proto->notify_authenticator) {
		...
		kobject_uevent_env(&dev->htcmode_sdev.this_device->kobj, KOBJ_CHANGE, envp);
		...
		snprintf(name_buf, sizeof(name_buf), "SWITCH_NAME=%s", htcmode_shortname);
		snprintf(state_buf, sizeof(state_buf), "SWITCH_STATE=%s",
			(atomic_read(&htc_mode_status) == HTC_MODE_RUNNING ? "projecting" :
			(atomic_read(&htc_mode_status) == DOCK_ON_AUTOBOT ? "online" : "offline")));
...
static int autobot_setup(struct htcmode_protocol *config)
{
	...
	dev->cand_sdev.minor = MISC_DYNAMIC_MINOR;
	ret = misc_register(&dev->cand_sdev);
	if (ret < 0) {
		pr_err("usb cand_sdev misc_register fail\n");
		goto err_free;
	}

	dev->htcmode_sdev.name = htcmode_shortname;
	dev->htcmode_sdev.minor = MISC_DYNAMIC_MINOR;
	ret = misc_register(&dev->htcmode_sdev);
	if (ret < 0) {
		pr_err("usb htcmode_sdev misc_register fail\n");
