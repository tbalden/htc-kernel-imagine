...
int enable_timeout_debug = 0;

void enable_dump_debug_reg(int enable)
{
	if (enable)
		enable_timeout_debug = 1;
	else
		enable_timeout_debug = 0;
}
EXPORT_SYMBOL(enable_dump_debug_reg);
...
int gpi_terminate_all(struct dma_chan *chan)
{
	...
	if (enable_timeout_debug) {
		gpi_dump_debug_reg(gpii);
		panic("i2c dma timeout");
	}
