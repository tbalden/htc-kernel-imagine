...
static void htc_show_sensor_temp(void)
{
	...
	int ret = 0;
	int temp = 0;
	char *sensors[] = {"emmc-therm-adc", "quiet-therm-adc", "msm-therm-adc",
			"xo-therm-adc", "pa-therm1-adc", "battery"};
	...
	for (i = 0; i < ARRAY_SIZE(sensors); i++) {
		...
		if (ret != 0)
			pr_info("[Power_FDA] %s: (%s) thermal_zone_get_temp() failed tzd->id=%d\n",
				__func__, tzd->type, tzd->id);
