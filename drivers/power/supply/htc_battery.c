...
static bool htcchg_entry_done = false;
...
//	pr_info("[AIC]scrn = %d, entry = %d\n", screen_on_cnt, entry_AIC_charing_cnt);
...
void update_htc_extension_state(void)
{
	...
	htc_batt_info.icharger->register_read(BATTERY_CHARGER_STATUS_2_REG, &reg);
	if (reg & CHARGER_ERROR_STATUS_SFT_EXPIRE_BIT)
...
static int ibat_map_typeC[MAX_IDX][HEALTH_LEVELS] = {
...
/* IBAT_IDX =  < VOLTAGE-LIMITED, DISPLAY-ON > */
/*  #0 (00) */ { 1300,  1300,  IBAT_MAX, IBAT_MAX,  1800,  1300},
/*  #1 (01) */ { 1000,  1000,      1000,     1000,  1000,  1000},
/*  #2 (10) */ {  250,   500,      1800,     1800,  1800,     0},
/*  #3 (11) */ {  250,   500,      1000,     1000,  1000,     0},
};
...
int update_ibat_setting (void)
{
	...
	if (is_vol_limited || batt_vol > thermal_limit_vol[batt_thermal])
		is_vol_limited = true;
	if (!is_vol_limited || batt_vol < thermal_limit_vol_recover[batt_thermal])
		is_vol_limited = false;

	/* Step#3: Apply Screen ON configuartion */
	is_screen_on = !(htc_batt_info.state & STATE_SCREEN_OFF);

	if (is_screen_on && (strcmp(htc_get_bootmode(), "") == 0) &&
	...
	idx = ((2 * (is_vol_limited ? 1 : 0)) + (1 * (is_screen_on ? 1 : 0)));

	switch (chg_type) {
		case POWER_SUPPLY_TYPE_USB_HVDCP_3:
			if (batt_temp > 450)
				over_cool_chg = true;
			if (!not_entry_again && !over_cool_chg)
				idx = idx & 0x2;
		...
		case POWER_SUPPLY_TYPE_USB_PD:
			...
			break;
		case POWER_SUPPLY_TYPE_TYPEC:
			ibat_ma = ibat_map_typeC[idx][batt_thermal];
			break;
		default:
			ibat_ma = ibat_max_ma;
			break;
	...
	if ((g_thermal_limit_ma > 0) && (ibat_ma > g_thermal_limit_ma)) {
        ...
        BATT_LOG("%s: batt_thermal=%d,is_vol_limited(>%dmV)=%d,is_screen_on=%d,idx=%d,cc_exit=%d,ibat_ma=%d\n",
				__func__, batt_thermal, thermal_limit_vol[batt_thermal],
				is_vol_limited, is_screen_on, idx, not_entry_again, ibat_ma);
...
#if defined(CONFIG_FB)
#define HTCCHG_SCREEN_OFF_DELAY_MS	300000
...
#endif
					if (delayed_work_pending(&htc_batt_info.htcchg_screen_work))
						cancel_delayed_work(&htc_batt_info.htcchg_screen_work);
                                break;
...
#define VUSB_INIT_MV 		4400
#define VUSB_HIGH_BOUND_MV 	4600
#define VUSB_LOW_BOUND_MV 	0
#define IBAT_HIGH_BOUND_MA 	2750
#define IBAT_LOWER_BOUND_MA 	2300
#define LEAVE_COOL_HIGH_TEMP	450
#define RE_ENTRY_COOL_HIGH_TEMP	430
#define ENTRY_COOL_TEMP_SRN_ON	370
...
static void htcchg_cc_leave(void)
{
	...
	// 1. Turn on HVDCP
	htc_batt_info.icharger->register_write(USBIN_OPTIONS_1_CFG_REG,
...
static void htcchg_cc_entry(void)
{
	...
	// 1. Turn off HVDCP daemon for preventing Vbus change
...
static void htcchg_check_cc_status(void)
{
	...
	if(htcchg_entry_done){
		...
		if(htc_batt_info.rep.charging_source != POWER_SUPPLY_TYPE_USB_HVDCP_3){
			...
			htcchg_cc_leave();
		}else if (htc_batt_info.rep.chg_en == 0){
			pr_info("[CC] Leave chg_en: %d [%d]\n", htc_batt_info.rep.batt_vol, htc_batt_info.rep.chg_en);
			re_entry = false;
			htcchg_cc_leave();
		}
	}else{
		if((htc_batt_info.rep.charging_source == POWER_SUPPLY_TYPE_USB_HVDCP_3) &&
				g_is_quick_charger && !htcchg_on && (htc_batt_info.rep.chg_en != 0)){
			if((htc_batt_info.rep.level >= ENTRY_SOC_MIN) && (htc_batt_info.rep.level <= ENTRY_SOC_MAX)){
				is_screen_on = !(htc_batt_info.state & STATE_SCREEN_OFF);
				if((htc_batt_info.rep.batt_temp > ENTRY_COOL_TEMP_SRN_ON/*entry_cool_temp*/) &&
						(htc_batt_info.rep.batt_temp < LEAVE_COOL_HIGH_TEMP) && is_screen_on){
...
static void htcchg_vbus_adjust_worker(struct work_struct *work)
{
	...
	int ibat_for_cc_check = get_property(htc_batt_info.batt_psy,POWER_SUPPLY_PROP_HTCCHG_CHARGE_NOW);
	...
	if (vbus_for_cc_check >= VUSB_LOW_BOUND_MV && vbus_for_cc_check <= VUSB_HIGH_BOUND_MV) {
		vbus_cnt = 0;
		if (ibat_for_cc_check < IBAT_LOWER_BOUND_MA) {
...
static void htcchg_init_worker(struct work_struct *work)
{
	...
	int vbat_for_cc_init = get_property(htc_batt_info.bms_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW)/1000;
	...
	if (vbat_for_cc_init > ENTRY_VBAT_MAX) {
		pr_info("[CC] Vbat too high, leave..., vbat = %d\n", vbat_for_cc_init);
		init_cnt = 0;
		not_entry_again = true;
		htcchg_on = false;
		htcchg_cc_leave();
...
static void htcchg_screen_worker(struct work_struct *work)
{
	if ((htc_batt_info.rep.charging_source == POWER_SUPPLY_TYPE_USB_HVDCP_3) &&
				(htc_batt_info.state & STATE_SCREEN_OFF))
	{
		pr_info("[CC][SCREEN] Screen OFF! Leaving Cool Charger!\n");
		htcchg_cc_leave();
	}
}
...
static void batt_worker(struct work_struct *work)
{
	...
	*/
	...
	if ((int)htc_batt_info.rep.charging_source > POWER_SUPPLY_TYPE_BATTERY) {
		...
		if (htc_batt_info.prev.charging_source != htc_batt_info.rep.charging_source || s_first) {
			...
			if (delayed_work_pending(&htc_batt_info.screen_ibat_limit_enable_work))
				cancel_delayed_work(&htc_batt_info.screen_ibat_limit_enable_work);
			if (delayed_work_pending(&htc_batt_info.htcchg_screen_work))
				cancel_delayed_work(&htc_batt_info.htcchg_screen_work);
			update_htc_extension_state();
		}
	...
	BATT_EMBEDDED("ID=%d,"
		...
		get_property(htc_batt_info.batt_psy,POWER_SUPPLY_PROP_HTCCHG_CHARGE_NOW),
		get_property(htc_batt_info.usb_psy,POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED)/1000,
	...
	if (aicl <= 25 && htc_batt_info.rep.overload == 1) {
		htc_batt_info.icharger->register_write(USBIN_AICL_OPTIONS_CFG_REG,
						SUSPEND_ON_COLLAPSE_USBIN_BIT, 0);
		msleep(50);
		htc_batt_info.icharger->register_write(USBIN_AICL_OPTIONS_CFG_REG,
						SUSPEND_ON_COLLAPSE_USBIN_BIT, SUSPEND_ON_COLLAPSE_USBIN_BIT);
		// rerun AICL
		htc_batt_info.icharger->register_write(CMD_HVDCP_2_REG, TRIGGER_AICL_BIT, TRIGGER_AICL_BIT);
	}
			...
			char *buf)
{
	...
	int isen_value, isen_value_adc, batt_res;

	time_to_tm(t, 0, &timeinfo);

	usb_conn_temp = get_property(htc_batt_info.batt_psy, POWER_SUPPLY_PROP_USB_CONN_TEMP);

	isen_value = get_property(htc_batt_info.batt_psy, POWER_SUPPLY_PROP_HTCCHG_CHARGE_NOW);
	isen_value_adc = get_property(htc_batt_info.batt_psy, POWER_SUPPLY_PROP_HTCCHG_CHARGE_NOW_RAW);
...
static int htc_battery_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&htc_batt_info.htcchg_screen_work, htcchg_screen_worker);
	...
	if (node) {
		if (!of_device_is_available(node)) {
			...
			if (ret)
				...
				ret = of_property_read_u32_array(node, "htc,typec-chg-fcc-limits",
								 &ibat_map_typeC[0][0], MAX_IDX * HEALTH_LEVELS);
				if (ret)
					pr_err("%s: error reading type-C thermal table.\n", __func__);
				// read vbat threshold
