...
{
	int maxSize = 0;
	if (!main_display || !main_display->modes)
		pr_warn("%s: Can not access DRM display\n", __func__);
	else {
		pr_info("[mirrorlink] device(%d, %d)\n",
			main_display->modes->timing.h_active,
			main_display->modes->timing.v_active);
		maxSize = main_display->modes->timing.h_active * main_display->modes->timing.v_active;
	}
