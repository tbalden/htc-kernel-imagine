...
/* Force disable SMLog for 845 CPU V1.0 HW devices.
   Modem MPU partition is not enough.
   Device will hit TZ abort if SMlog enalbed with 845 V1.0 CPU */
extern bool is_845_v1_hw(void);
			...
			size_t count)
{
	...
	if (is_845_v1_hw()) {
		pr_err("[rtel] Disable SMlog due to the CPU is 845 v1.0");
		return count;
	}
