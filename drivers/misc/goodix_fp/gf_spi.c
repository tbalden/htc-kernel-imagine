...
#define HTC_DISABLE_INPUT_DEVICE    1 /* HTC : Disable register input device
                                               use uinput instead         */
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
...
#ifdef CONFIG_HTC_PCN_HAL_FOOTPRINT_CMD
    #define TIME_STRING_SIZE 128
    char time_string[TIME_STRING_SIZE] = {0,};
#endif /* CONFIG_HTC_PCN_HAL_FOOTPRINT_CMD */
	...
	case GF_IOC_CHIP_INFO:
		...
		break;
#ifdef CONFIG_HTC_PCN_HAL_FOOTPRINT_CMD
    case HTC_HAL_FOOTPRINT:
        if (copy_from_user(&time_string ,(char *)arg, sizeof(char)*TIME_STRING_SIZE)) {
            pr_err(HTC_TIME_TAG "%s : Fail to copy String from HAL!\n",__func__);
            retval = -EFAULT;
            break;
        }
        pr_info(HTC_TIME_TAG "%s : Android-Time = %s\n",__func__,time_string);
        break;
#endif //CONFIG_HTC_PCN_HAL_FOOTPRINT_CMD
...
static int gf_open(struct inode *inode, struct file *filp)
{
	...
	if (status == 0) {
		if (status == 0) {
            ...
            // HAL will send GF_IO_RESET ioctl to kernel, do not do hw_reset here.
			//gf_hw_reset(gf_dev, 3);
			gf_dev->device_available = 1;
		}
	} else {
...
#endif
{
...
#ifndef HTC_DISABLE_INPUT_DEVICE
    ...
    status = htc_hw_reset_init(&gf_dev->spi->dev);
    if (status) {
        pr_err("%s : fail to init hw_reset pin, status = %d\n",__func__,status);
    }
#endif //CONFIG_HTC_PCN_FP_ENABLE_HW_RESET_PINCTRL

#ifdef CONFIG_HTC_PCN_FP_ENABLE_3V3_POWER
    status = enable_goodix_power(gf_dev);
    if (status){
        pr_err("%s : failed to enable 3v3 power, status = %d\n",__func__,status);
    }
