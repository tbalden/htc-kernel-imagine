...
static int qpnp_rgb_init(struct qpnp_led_data *led)
{
	...
	rc = qpnp_pwm_init(led->rgb_cfg->pwm_cfg, led->pdev,
				led->cdev.name);
	if (rc) {
		dev_err(&led->pdev->dev,
				...
				struct device_node *node)
{
	...
	if (pwm_cfg->mode == LPG_MODE || pwm_cfg->use_blink) {
		...
		prop = of_find_property(node, "qcom,duty-pcts",
			&pwm_cfg->duty_cycles->num_duty_pcts);
		if (!prop) {
			dev_err(&pdev->dev, "Looking up property " \
				"node qcom,duty-pcts failed\n");
			rc =  -ENODEV;
			goto bad_lpg_params;
		} else if (!pwm_cfg->duty_cycles->num_duty_pcts) {
			dev_err(&pdev->dev, "Invalid length of " \
				"duty pcts\n");
			rc =  -EINVAL;
			goto bad_lpg_params;
		}
...
bad_lpg_params:
	...
	if (pwm_cfg->mode == PWM_MODE) {
		dev_err(&pdev->dev, "LPG parameters not set for" \
				...
				struct device_node *node)
{
	...
	if (!rc) {
		...
		if ((led_mode == MANUAL_MODE) || (led_mode == -EINVAL)) {
			dev_err(&led->pdev->dev, "Selected mode not " \
				"supported for rgb.\n");
			return -EINVAL;
		}
...
void set_led_touch_solution(uint16_t solution)
{
	...
	for_each_child_of_node(g_led_virtual->pdev->dev.of_node, temp) {
...
static void led_blink_work_func(struct work_struct *work)
{
	...
	if (ldata->id == QPNP_ID_LED_MPP) {
		...
		if (rc < 0)
			dev_err(&ldata->pdev->dev, "MPP set brightness failed (%d)\n", rc);
	} else {
