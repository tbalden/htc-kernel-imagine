...
struct delayed_work sdm845_delayed_work;
static struct workqueue_struct *sdm845_work_queue;
static int flashlight_turn_off(void);

int sdm845_flash_mode(int, int);
int sdm845_torch_mode(int, int);
...
static int qpnp_flash_led_switch_set(struct flash_switch_data *snode, bool on)
{
	...
	int rc, i, addr_offset;
	u8 val, mask;
	u8 flash_total = 0, torch_total = 0;
	...
	for (i = 0; i < led->num_fnodes; i++) {
		...
		if(led->fnode[i].led_on && led->fnode[i].type == FLASH_LED_TYPE_FLASH)
			flash_total++;
		if(led->fnode[i].led_on && led->fnode[i].type == FLASH_LED_TYPE_TORCH)
			torch_total++;
	...
	if (flash_total > torch_total)
	{
		if(set_backlight)
			set_backlight(BACKLIGHT_OFF);
		/*** software timer ***/
		queue_delayed_work(sdm845_work_queue, &sdm845_delayed_work, msecs_to_jiffies(FLASH_TIME_OUT));
		flash_total = 0;
		torch_total = 0;
	}
	return 0;
...
int sdm845_flash_mode(int led0_curr, int led1_curr)
{
...
int sdm845_torch_mode(int led0_curr, int led1_curr)
{
						...
						const char *buf, size_t count)
{
	...
	sdm845_flash_mode(led0_curr, led1_curr);
						...
						const char *buf, size_t count)
{
	...
	sscanf(buf, "%d %d", &led0_curr, &led1_curr);
	sdm845_torch_mode(led0_curr, led1_curr);
...
#endif
	this_led =led;
	INIT_DELAYED_WORK(&sdm845_delayed_work, flashlight_turn_off_work);
	sdm845_work_queue = create_singlethread_workqueue("sdm845_wq");
	if (!sdm845_work_queue)
		goto err_create_sdm845_work_queue;

	htc_flash_main = &sdm845_flash_mode;
	htc_torch_main = &sdm845_torch_mode;
