...
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
		if (nanohub_edge_vol_wakeup(ddata->wakeup_bitmask) < 0)
			KEY_LOGE("%s: nanohub_edge_vol_wakeup ON fail!\n", __func__);
#endif
		ddata->set_wakeup = 1;
...
#ifdef CONFIG_NANOHUB_EDGE_ULTRA
		if (nanohub_edge_vol_wakeup(ddata->wakeup_bitmask) < 0)
			KEY_LOGE("%s: nanohub_edge_vol_wakeup OFF fail!\n", __func__);
#endif
...
static ssize_t disable_interrupt_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	unsigned char input = 0;
	struct gpio_keys_drvdata *ddata = dev_get_drvdata(dev);
	if (sscanf(buf, "%hhu", &input) != 1)
		return -EINVAL;
	if (input > 7) {
		KEY_LOGE("Invalid value %d\n", input);
		return -EINVAL;
	}
	if ((ddata->disable_interrupt ^ input) & BIT(0)) {
		if (input & BIT(0)) {
			disable_irq(vol_down_irq);
			KEY_LOGI("Disable vol_down_irq\n");
		} else {
			enable_irq(vol_down_irq);
			KEY_LOGI("Enable vol_down_irq\n");
		}
	}
	if ((ddata->disable_interrupt ^ input) & BIT(1)) {
		if (input & BIT(1)) {
			disable_irq(vol_up_irq);
			KEY_LOGI("Disable vol_up_irq\n");
		} else {
			enable_irq(vol_up_irq);
			KEY_LOGI("Enable vol_up_irq\n");
		}
	}
	if ((ddata->disable_interrupt ^ input) & BIT(2)) {
		if (input & BIT(2)) {
			disable_irq(power_irq);
			KEY_LOGI("Disable power_irq\n");
		} else {
			enable_irq(power_irq);
			KEY_LOGI("Enable power_irq\n");
		}
	}
	ddata->disable_interrupt = input;

	return count;
}

static ssize_t disable_interrupt_show(struct device *dev,
					struct device_attribute *attr, char *buf)
{
	struct gpio_keys_drvdata *ddata = dev_get_drvdata(dev);

	return scnprintf(buf, PAGE_SIZE, "%d\n", ddata->disable_interrupt);
}

static DEVICE_ATTR(disable_interrupt, S_IWUSR | S_IWGRP | S_IRUGO,
					disable_interrupt_show, disable_interrupt_store);
...
static struct attribute *gpio_keys_attrs[] = {
	...
	&dev_attr_disable_interrupt.attr,
	NULL,
};
...
#ifdef CHECK_IRQ_FLAGS
static void gpio_key_set_irq_flag(unsigned int gpio)
{
	int state = gpio_get_value(gpio);
	if(state) {
		irq_set_irq_type(gpio_to_irq(gpio), IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND);
	} else {
		irq_set_irq_type(gpio_to_irq(gpio), IRQF_TRIGGER_RISING | IRQF_NO_SUSPEND);
	}
	KEY_LOGD("Set gpio%d irq type to IRQF_TRIGGER_%s\n", gpio, state ? "FALLING" : "RISING");
}
#endif
...
#ifdef CHECK_IRQ_FLAGS
	gpio_key_set_irq_flag(bdata->button->gpio);
#endif
