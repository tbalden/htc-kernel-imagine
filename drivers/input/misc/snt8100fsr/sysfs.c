...
static uint32_t fw_update_status_variable = 0;
static uint32_t fw_update_progress_variable = 0;
...
#else
        PRINT_NOTICE("ERROR. Only one byte allowed %d", count);
#endif
        return -1;
    }
 ...
 */
///DEBUG
int SNT_D1TEST_LIMIT = 64*1024;
module_param(SNT_D1TEST_LIMIT, int, S_IRUSR | S_IWUSR);
...
#ifdef CONFIG_HTC_FEATURE
        if (value < 0) {
            PRINT_CRIT("Invalid parameter %ld", value);
            return -EINVAL;
        }
#else
...
#ifdef CONFIG_HTC_FEATURE
///DEBUG
        if (value > SNT_D1TEST_LIMIT)
            value = SNT_D1TEST_LIMIT;
        PRINT_INFO("d1test frames %d", (int)value);
#endif
        if(!snt8100fsr_g->sc_cmd_buf && value != 0) {
            snt8100fsr_g->sc_cmd_buf = memory_allocate(sizeof(struct sc_command), GFP_DMA);
            if (snt8100fsr_g->sc_cmd_buf == NULL) {
                PRINT_CRIT("memory_allocate(sc_cmd_buf) failed");
                return -ENOMEM;
            }
        }
        snt8100fsr_g->sc_cmd_buf_ready = false;
...
ssize_t sysfs_product_config_show(struct device *dev,
    ...
    return ret;
#endif //#if !define(CONFIG_HTC_FEATURE)
}
...
ssize_t sysfs_fwupdate_store(struct device *dev,
    ...
    if (p_buf == NULL) {
        PRINT_ERR("Cannot allocate memory for input path!");
        return count;
    }
...
ssize_t fw_update_status_show(struct device *dev,
			struct device_attribute *attr,
			char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", fw_update_status_variable);
}

ssize_t fw_update_status_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf,
			size_t count)
{
	if (kstrtou32(buf, 10, &fw_update_status_variable) != 0)
		PRINT_CRIT("invalid parameter");

	return count;
}

ssize_t fw_update_timeout_show(struct device *dev,
			struct device_attribute *attr,
			char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", 210);
...
ssize_t fw_update_progress_show(struct device *dev,
			struct device_attribute *attr,
			char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", fw_update_progress_variable);
}

ssize_t fw_update_progress_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf,
			size_t count)
{
	if (kstrtou32(buf, 10, &fw_update_progress_variable) != 0)
		PRINT_CRIT("invalid parameter");

	return count;
}

ssize_t sysfs_flash_flag_show(struct device *dev,
                         struct device_attribute *attr,
                         char *buf)
{
        return snprintf(buf, PAGE_SIZE, "%d\n", snt8100fsr_g->flash_flag);
}

ssize_t sysfs_reg_bank_id_show(struct device *dev,
                         struct device_attribute *attr,
                         char *buf)
{
        if(snt8100fsr_g->weak_struct_flag)
            return snprintf(buf, PAGE_SIZE, "0x0000 0x0004\n");
        else
            return snprintf(buf, PAGE_SIZE, "0xFFFF 0xFFFF\n");
}
...
ssize_t sysfs_write_flash_reg_part_store(struct device *dev,
    ...
    PRINT_FUNC("%d bytes", count);
#endif
    enable_write_flash_reg_part_req(snt8100fsr_g, buf, count);
    PRINT_DEBUG("done.");
    return count;
}
...
ssize_t sysfs_set_sys_param_store(struct device *dev,
    ...
    snt8100fsr_g->set_sys_param_id  = val;
    ...
    snt8100fsr_g->set_sys_param_val = val;

    snt8100fsr_g->set_sys_param_status = -1;

    PRINT_DEBUG("SetSysParam(%d) = 0x%x",
                snt8100fsr_g->set_sys_param_id,
                snt8100fsr_g->set_sys_param_val);
                            ...
                            char *buf)
{
    ...
    if (snt8100fsr_g->get_sys_param_cmd && snt8100fsr_g->get_sys_param_cmd->length > 8) {
        ...
        PRINT_FUNC("id = %d, status=%d, len=%d", snt8100fsr_g->get_sys_param_id,
        ...
        PRINT_FUNC("id = %d, status=%d, val=%d", snt8100fsr_g->get_sys_param_id,
                                                snt8100fsr_g->get_sys_param_status,
...
ssize_t sysfs_get_sys_param_store(struct device *dev,
    ...
    snt8100fsr_g->get_sys_param_id  = val;
    snt8100fsr_g->get_sys_param_status = -1;

    PRINT_DEBUG("GetSysParam(%d)", snt8100fsr_g->set_sys_param_id);

    enable_get_sys_param(snt8100fsr_g);
...
#endif
    PRINT_FUNC("id = %d, len=%d", snt8100fsr_g->get_reg_id, snt8100fsr_g->get_reg_num);
...
#else
        PRINT_CRIT("memory_allocate(%d) failed",count);
#endif
        return -1;
    }
...
#endif
static DEVICE_ATTR(fw_update_status,
		SYSFS_PERM_SHOW_STORE,
		fw_update_status_show,
		fw_update_status_store);

static DEVICE_ATTR(fw_update_timeout,
		SYSFS_PERM_SHOW,
		fw_update_timeout_show,
		NULL);

static DEVICE_ATTR(fw_update_progress,
		SYSFS_PERM_SHOW_STORE,
		fw_update_progress_show,
		fw_update_progress_store);
...
static DEVICE_ATTR(flash_flag,
                   SYSFS_PERM_SHOW,
                   sysfs_flash_flag_show,
                   NULL);

static DEVICE_ATTR(reg_bank_id,
                   SYSFS_PERM_SHOW,
                   sysfs_reg_bank_id_show,
                   NULL);
...
static struct attribute *sysfs_attrs_static[] = {
    ...
    &dev_attr_fw_update_timeout.attr,
    &dev_attr_fw_update_progress.attr,
    &dev_attr_flash_flag.attr,
    &dev_attr_reg_bank_id.attr,
    NULL
};
...
int snt_sysfs_init(struct snt8100fsr *snt8100fsr, bool enable) {
    ...
    if (!enable) {
        ...
        if (sysfs_kobj_g != NULL) {
            htc_sysfs_init(snt8100fsr, sysfs_kobj_g, enable);
            sysfs_remove_link(sysfs_kobj_g, SYSFS_NAME);
            if (sysfs_attrs_g != NULL)
...
#ifdef CONFIG_HTC_FEATURE
    if (attrs == NULL) {
        PRINT_ERR("Cannot allocate memory, %d", __LINE__);
        return NULL;
    }
#endif
...
#ifdef CONFIG_HTC_FEATURE
        if (full_name == NULL) {
            PRINT_ERR("Cannot allocate memory for reg:%s, %d", reg_name, __LINE__);
            continue;
        }
#endif
