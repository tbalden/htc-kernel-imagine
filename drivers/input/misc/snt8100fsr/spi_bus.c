...
int snt_spi_wake_device(struct spi_device *spi) {
    ...
    t.speed_hz = (uint32_t)snt8100fsr_spi_g->spi_freq_khz * 1000u;
#else
    t.speed_hz = (uint32_t)snt8100fsr_g->spi_freq_khz * 1000u;
#endif
#endif
...
#ifdef CONFIG_NANOHUB_SILEGO
    snt8100fsr_spi_g = snt8100fsr;
    if(snt8100fsr_g)
        snt8100fsr_g->spi = spi;
#else
