...
#else
        PRINT_ERR("Possible Leak! Freeing 0x%p, %d bytes",
                  a->buf + sizeof(uint32_t),
                  a->size - get_memory_prefix_and_postfix_size());
#endif
...
void *memory_allocate(size_t size, gfp_t flags) {
...
#else
    PRINT_FUNC("%d bytes, 0x%08X", size, flags);
#endif
    ...
    if (buf == NULL) {
        ...
        return NULL;
#else
        PRINT_CRIT(OOM_STRING " %d bytes, 0x%08X", size, flags);
#endif
    }
    ...
    if (a == NULL) {
        ...
        return NULL;
#else
        PRINT_CRIT(OOM_STRING " %d bytes, 0x%08X",
                   sizeof(struct allocation), GFP_KERNEL);
#endif
    }
...
void memory_free(void *buf) {
    ...
    PRINT_DEBUG("Free'd 0x%p, %zu bytes", buf,
                size - get_memory_prefix_and_postfix_size());
#else
    PRINT_DEBUG("Free'd 0x%p, %d bytes", buf,
                size - get_memory_prefix_and_postfix_size());
#endif
}
