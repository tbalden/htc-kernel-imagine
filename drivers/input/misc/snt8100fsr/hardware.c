...
void load_registers_from_disk(struct snt8100fsr *snt8100fsr) {
    ...
    while (reg_name) {
#if defined(CONFIG_HTC_FEATURE)
        count += snprintf(full_path + count, PATH_MAX, REGISTER_FILES_PATH);
        count += snprintf(full_path + count, PATH_MAX, REGISTER_PREFIX);
        count += snprintf(full_path + count, PATH_MAX, reg_name);
#else
