...
extern uint8_t edge_debug_mask;
...
#define PRINT_EMERG(fmt, ...)	printk(KERN_EMERG TAG EMERG_STR	P_PRE fmt P_POST, __func__, ##__VA_ARGS__)
#define PRINT_ALERT(fmt, ...)	printk(KERN_ALERT TAG ALERT_STR	P_PRE fmt P_POST, __func__, ##__VA_ARGS__)
#define PRINT_CRIT(fmt, ...)	printk(KERN_CRIT TAG CRIT_STR	P_PRE fmt P_POST, __func__, ##__VA_ARGS__)
#define PRINT_ERR(fmt, ...)	printk(KERN_ERR TAG ERR_STR	P_PRE fmt P_POST, __func__, ##__VA_ARGS__)
#define PRINT_WARN(fmt, ...)	printk(KERN_WARNING TAG WARN_STR	fmt P_POST, ##__VA_ARGS__)
#define PRINT_NOTICE(fmt, ...)	printk(KERN_NOTICE TAG NOTICE_STR	fmt P_POST, ##__VA_ARGS__)
#define PRINT_INFO(fmt, ...)	printk(KERN_INFO TAG INFO_STR		fmt P_POST, ##__VA_ARGS__)
#define PRINT_DEBUG(fmt, ...)	do { \
		if (edge_debug_mask & BIT(0)) { \
			printk(KERN_DEBUG TAG DEBUG_STR	P_PRE fmt P_POST, __func__, ##__VA_ARGS__); \
		} \
	} while (0);
#define PRINT_FUNC(fmt, ...)	do { \
		if (edge_debug_mask & BIT(1)) { \
			printk(KERN_DEBUG TAG FUNC_STR	P_PRE fmt P_POST, __func__, ##__VA_ARGS__); \
		} \
	} while (0);
