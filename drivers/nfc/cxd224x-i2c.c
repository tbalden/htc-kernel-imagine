...
static void cxd224x_workqueue(struct work_struct *work)
{
	...
	gpio_set_value(cxd224x_dev->rst_gpio, 0);	//for gpio_30 with inverter, trigger reset
	...
	gpio_set_value(cxd224x_dev->rst_gpio, 1);	//for gpio_30 with inverter, cancel reset
}
...
int nci_Reader(control_msg_pack *script, unsigned int scriptSize) {
	...
	if ( cxd224x_RxData2(nci_header, 3) < 0) {
		I("I2C error while read out the NCI header.\n");
		return -255;
	} else {
		memcpy(nci_rx_header,nci_header,3);
		I("@@@1 NCI header read: [Header] 0x%02X, 0x%02X\n [Len] 0x%02X", nci_rx_header[0], nci_rx_header[1], nci_rx_header[2]);

		mdelay(NFC_READ_DELAY);
		if ( (int)nci_rx_header[2] < 0) {
			I("I2C error while read out the NCI data length.\n");
			return -255;
		} else {		
			//I("NCI data length read: %d\n", (int)nci_rx_header[2]);
			mdelay(NFC_READ_DELAY);
			if ( cxd224x_RxData2(receiverBuffer, (int)nci_rx_header[2]) < 0) {
				I("I2C error while read out the NCI data.\n");
				return -255;
			} else {
				I("NCI data: ");
				nfc_nci_dump_data(receiverBuffer, (int)nci_rx_header[2]);
			}
		}
	}
	...
	I("### nci_data_len:%d ", (int)nci_rx_header[2]);
		...
		case 0x0103:
			...
			if (*(script->exp_ntf)) { /*Case when expected multiple remote SE NTF coming*/
				if (memcmp(nci_rx_header, expect_ntf_header, 2) == 0){
					...
					if (receiverBuffer[(int)nci_rx_header[2] - 1] == 0) {
...
static void cxd224x_pon_on(void)
{
	...
	mdelay(1000);
	I("%s : after set fel_pon output HIGH (%d) DELAY 1000\n", __func__, gpio_get_value(cxd224x_dev->wake_gpio));
	mdelay(100);
...
static int cxd224x_parse_dt(struct device *dev, struct cxd224x_platform_data *pdata)
{
	...
	if (!gpio_is_valid(pdata->rst_gpio))
        ...
        pdata->sony_rfs = of_get_named_gpio(dt, "sony,rfs", 0);
        if (!gpio_is_valid(pdata->sony_rfs))
                E("DT:pdata->rfs value is not valid\n");
        else
                I("DT:pdata->frs=%d\n", pdata->sony_rfs);
#endif
			   ...
			   const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "%s, xrst deassert\n", __func__);
        ret = gpio_request_one(platform_data->sony_rfs, GPIOF_OUT_INIT_HIGH, "nfc_rfs");
        if (ret)
                goto err_exit;
        ret = gpio_direction_output(platform_data->sony_rfs, 1);
        if (ret)
            return -ENODEV;
        dev_info(&client->dev, "%s, pull high rfs\n", __func__);
