...
#ifdef SDM845_NFC_POWER_CONTROL
struct regulator *nfc_pm8998_lvs1;
#endif //SDM845_NFC_POWER_CONTROL
...
#ifdef HTC_NFC_PN553
void pn544_Enable(void)
{
	struct nqx_dev *pni = pn_info;
	I("%s: gpio=%d set_value=%d\n", __func__, pni->en_gpio, 1);

	gpio_set_value(pni->en_gpio, 1);
	pni->nfc_ven_enabled = 1;
}

void pn544_Disable(void)
{
	struct nqx_dev *pni = pn_info;
	I("%s: gpio=%d set_value=%d\n", __func__, pni->en_gpio, 0);

	gpio_set_value(pni->en_gpio, 0);
	pni->nfc_ven_enabled = 0;
}

int pn544_irq_gpio(void) {
	struct nqx_dev *pni = pn_info;
	return gpio_get_value(pni->irq_gpio);
}

int pn544_RxData(uint8_t *rxData, int length)
{
	...
	for (loop_i = 0; loop_i < NFC_I2C_RETRY; loop_i++) {
		D("%s: retry %d ........\n", __func__, loop_i);
		if (i2c_master_recv(pni->client, rxData, length) > 0)
			break;
		mdelay(10);
	}

	if (loop_i >= NFC_I2C_RETRY) {
		E("%s Error in i2c read: retry over %d\n", __func__,
			NFC_I2C_RETRY);
		return -EIO;
	}
...
int pn544_TxData(uint8_t *txData, int length)
{
	...
	for (loop_i = 0; loop_i < NFC_I2C_RETRY; loop_i++) {
		D("%s: retry %d ........\n", __func__, loop_i);
		if (i2c_master_send(pni->client, txData, length) > 0)
			break;
		msleep(10);
	}

	if (loop_i >= NFC_I2C_RETRY) {
		E("%s Error in i2c write: retry over %d\n", __func__,
			NFC_I2C_RETRY);
		return -EIO;
	}
...
{
	int ret = 0;
	I("%s mfg_nfc_cmd_result is %d\n", __func__, mfg_nfc_cmd_result);
	ret = snprintf(buf, MAX_BUFFER_SIZE*2 , "%d\n\n", mfg_nfc_cmd_result);
	return ret;
}
				...
				const char *buf, size_t count)
{
	int code = -1;
	sscanf(buf, "%d", &code);
	if (nqx_clock_select(pn_info) < 0)
		E("%s unable to enable clock\n", __func__);
	msleep(20);
	mfg_nfc_cmd_result = mfg_nfc_test(code);
	if (nqx_clock_deselect(pn_info) < 0)
			...
			struct device_attribute *attr, char *buf)
{
	...
	if (nqx_clock_select(pn_info) < 0)
		E("%s unable to enable clock\n", __func__);
	msleep(20);
	mfg_nfc_cmd_result = mfg_nfc_test(0);
	if (nqx_clock_deselect(pn_info) < 0)
		E("%s unable to disable clock\n",__func__);
	if (mfg_nfc_cmd_result > 0) {
		return scnprintf(buf, PAGE_SIZE,
			"NFC firmware version: 0x%07x\n", mfg_nfc_cmd_result);
	}
	else if (mfg_nfc_cmd_result == 0) {
		return scnprintf(buf, PAGE_SIZE,
			...
			struct device_attribute *attr, char *buf)
{
	if (nqx_clock_select(pn_info) < 0)
		E("%s unable to enable clock\n", __func__);
	msleep(20);
	mfg_nfc_cmd_result = mfg_nfc_test(1);
	if (nqx_clock_deselect(pn_info) < 0)
		E("%s unable to disable clock\n",__func__);
	if (mfg_nfc_cmd_result == 1) {
		return scnprintf(buf, PAGE_SIZE,
			"%s Succeed\n",__func__);
	}
	else if (mfg_nfc_cmd_result == 0) {
			...
			struct device_attribute *attr, char *buf)
{
	if (nqx_clock_select(pn_info) < 0)
		E("%s unable to enable clock\n", __func__);
	msleep(20);
	mfg_nfc_cmd_result = mfg_nfc_test(2);
	if (nqx_clock_deselect(pn_info) < 0)
		E("%s unable to disable clock\n",__func__);
	if (mfg_nfc_cmd_result == 1) {
		return scnprintf(buf, PAGE_SIZE,
...
#if FTM_NFC_CPLC
static ssize_t mfg_nfc_cplc(struct device *dev,
			struct device_attribute *attr, char *buf)
{
...
static ssize_t mfg_nfc_dpc(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	char* tmp_cplc  = mfg_nfc_dpc_result();
	if (tmp_cplc != NULL) {
		return scnprintf(buf, PAGE_SIZE,
			"OUTPUT:\n%s\n",tmp_cplc);
	}
...
static DEVICE_ATTR(nxp_chip_alive, 0440, nxp_chip_alive_show, NULL);
...
#if FTM_NFC_CPLC
static DEVICE_ATTR(mfg_nfc_cplc, 0440, mfg_nfc_cplc, NULL);
static DEVICE_ATTR(mfg_nfc_dpc, 0440, mfg_nfc_dpc, NULL);
#endif //FTM_NFC_CPLC
#endif
...
static int nfc_parse_dt(struct device *dev, struct nqx_platform_data *pdata)
{
...
#ifdef HTC_NFC_PN553
	is_alive = pn553_htc_check_rfskuid(is_alive);
	dev_err(dev,
			"[NFC] RFSKUID check is_alive = %d\n",is_alive);
#endif //HTC_NFC_PN553
	...
	pdata->wakeup_gpio = of_get_named_gpio(np, "qcom,nq-wakeup", 0);
        if (!gpio_is_valid(pdata->wakeup_gpio)) {
                dev_warn(dev,
                        "wakeup GPIO <OPTIONAL> error getting from OF node\n");
                pdata->wakeup_gpio = -EINVAL;
        }
			...
			const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(platform_data->wakeup_gpio)) {
                r = gpio_request(platform_data->wakeup_gpio, "nfc_wakeup_gpio");
                if (r) {
                        dev_err(&client->dev,
                        "%s: unable to request nfc wakeup gpio [%d]\n",
                                __func__,
                                platform_data->wakeup_gpio);
                        goto err_mem;
                }
                r = gpio_direction_output(platform_data->wakeup_gpio, 0);
                if (r) {
                        dev_err(&client->dev,
                                "%s: unable to set direction for nfc wakeup gpio [%d]\n",
                                        __func__,
                                        platform_data->wakeup_gpio);
                        goto err_wakeup_gpio;
                }
        } else {
                dev_err(&client->dev,
                "%s: nfc wakeup gpio not provided\n", __func__);
                goto err_mem;
        }

	//gpio_is_valid used int as par but our pin defined as unsign,
	//gpio_request also take unsigned par
        //markout the gpio_is_valid condition ++
	...
	//mark the gpio_is_valid condition --
	...
	nqx_dev->wakeup_gpio = platform_data->wakeup_gpio;
	...
	pn_info = nqx_dev;
	nqx_dev->boot_mode = pn553_htc_get_bootmode();
#else //HTC_NFC_PN553
...
#ifdef HTC_NFC_PN553
	nqx_dev->pn544_class = class_create(THIS_MODULE, "NFC_sensor");
	if (IS_ERR(nqx_dev->pn544_class)) {
		r = PTR_ERR(nqx_dev->pn544_class);
		nqx_dev->pn544_class = NULL;
		E("%s : class_create failed\n", __func__);
		goto err_platform_data;
	}

	nqx_dev->pn_dev = device_create(nqx_dev->pn544_class, NULL, 0, "%s", "pn544");
	if (unlikely(IS_ERR(nqx_dev->pn_dev))) {
		r = PTR_ERR(nqx_dev->pn_dev);
		nqx_dev->pn_dev = NULL;
		E("%s : device_create failed\n", __func__);
		goto err_platform_data;
	}

	nqx_dev->comn_dev = device_create(nqx_dev->pn544_class, NULL, 0, "%s", "comn");
	if (unlikely(IS_ERR(nqx_dev->comn_dev))) {
		r = PTR_ERR(nqx_dev->comn_dev);
		nqx_dev->comn_dev = NULL;
		E("%s : device_create failed\n", __func__);
		goto err_platform_data;
	}

	I("%s: device_create_file for FTM mode+\n", __func__);
	r = device_create_file(nqx_dev->comn_dev, &dev_attr_mfg_nfc_ctrl);
	if (r) {
		E("pn544_probe device_create_file dev_attr_mfg_nfc_ctrl failed\n");
	}

	r = device_create_file(nqx_dev->comn_dev, &dev_attr_mfg_nfcversion);
	if (r) {
		E("pn544_probe device_create_file dev_attr_mfg_nfcversion failed\n");
	}

	r = device_create_file(nqx_dev->comn_dev, &dev_attr_mfg_nfcreader);
	if (r) {
		E("pn544_probe device_create_file dev_attr_mfg_nfcreader\n");
	}

	r = device_create_file(nqx_dev->comn_dev, &dev_attr_mfg_nfccard);
	if (r) {
		E("pn544_probe device_create_file dev_attr_mfg_nfccard failed\n");
	}

	r = device_create_file(nqx_dev->comn_dev, &dev_attr_mfg_nfc_cplc);
	if (r) {
		E("pn544_probe device_create_file dev_attr_mfg_nfc_cplc failed\n");
	}

	r = device_create_file(nqx_dev->comn_dev, &dev_attr_mfg_nfc_dpc);
	if (r) {
		E("pn544_probe device_create_file dev_attr_mfg_nfc_dpc failed\n");
	}

    I("%s: device_create_file for FTM mode done -\n", __func__);
	if (!is_alive) {
		I("%s: Without NFC, device_create_file dev_attr_nxp_chip_alive \n", __func__);
		r = device_create_file(nqx_dev->pn_dev, &dev_attr_nxp_chip_alive);
		if (r) {
			E("pn544_probe device_create_file dev_attr_nxp_chip_alive failed\n");
		}
	} else {
        //Disable NFC if it is not off-mode charging
		if (nqx_dev->boot_mode == NFC_BOOT_MODE_OFF_MODE_CHARGING) {
			I("%s: NFC_BOOT_MODE_OFF_MODE_CHARGING (bootmode = %d)\n", __func__, nqx_dev->boot_mode);
			mfg_nfc_test(88);
...
err_wakeup_gpio:
	gpio_free(platform_data->wakeup_gpio);
err_ese_gpio:
