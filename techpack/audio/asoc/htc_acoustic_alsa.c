...
static struct wakeup_source htc_acoustic_wakelock;
extern struct wakeup_source compr_lpa_q6_cb_wakelock;
static struct wakeup_source htc_acoustic_wakelock_timeout;
struct avcs_ctl {
       atomic_t ref_cnt;
       void *apr;
};
...
EXPORT_SYMBOL(htc_acoustic_register_ops);
...
void htc_set_q6_effect(int port_id, int topology) {
	if(the_ops && the_ops->set_q6_effect)
		the_ops->set_q6_effect(port_id, topology);
	else
		pr_warn("%s set_q6_effect not registered!\n", __func__);
}
EXPORT_SYMBOL(htc_set_q6_effect);
	...
	switch (cmd) {
		...
		case ACOUSTIC_UPDATE_BEATS_STATUS: {
			if(sizeof(s32_value) <= us32_size) {
				...
				//edev_beats.state = -1;
				htc_misc_extcon_set_state(EXTCON_AUDIO_BEATS, s32_value);
			} else {
		...
		case ACOUSTIC_UPDATE_DQ_STATUS: {
			if(sizeof(s32_value) <= us32_size) {
				...
				//edev_dq.state = -1;
				htc_misc_extcon_set_state(EXTCON_AUDIO_DQ, s32_value);
			} else {
		...
		case ACOUSTIC_NOTIFIY_FM_SSR: {
			...
			//edev_fm.state = -1;
			htc_misc_extcon_set_state(EXTCON_AUDIO_FM, s32_value);
			break;
		}
#endif
		case ACOUSTIC_CONTROL_WAKELOCK: {
			if(sizeof(s32_value) <= us32_size) {
				...
				if (s32_value == 1) {
					__pm_wakeup_event(&htc_acoustic_wakelock, jiffies_to_msecs(60*HZ));
					__pm_relax(&compr_lpa_q6_cb_wakelock);
				} else {
					__pm_wakeup_event(&htc_acoustic_wakelock_timeout, jiffies_to_msecs(1*HZ));
					__pm_relax(&htc_acoustic_wakelock);
				}
			} else {
...
static int __init acoustic_init(void)
{
	...
	wakeup_source_init(&htc_acoustic_wakelock,  "htc_acoustic");
	wakeup_source_init(&htc_acoustic_wakelock_timeout, "htc_acoustic_timeout");
