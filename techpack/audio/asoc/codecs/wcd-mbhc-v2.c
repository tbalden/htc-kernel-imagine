...
/* HTC_AUD_START */
#include <linux/extcon/extcon-htc-misc.h>

#define WCD934X_CDC_RX1_RX_PATH_CFG0  0xB56
#define WCD934X_CDC_RX2_RX_PATH_CFG0  0xB6A
/* add debounce for LR-Mic switch to avoid pop from ANC feature*/
#define HTC_ANC_SWITCH5_DEBOUNCE_TIME 200000
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA4476) {
		pr_info("%s headset switch S3 status [PMI8998 GPIO%d: %d]\n", __func__,
			hs_cfg.fsa_switch_gpio[FSA4476_EN2],
			gpio_get_value(hs_cfg.fsa_switch_gpio[FSA4476_EN2]));
	} else {
	...
	if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA4476) {
		pr_info("%s headset USB FSA4476 ENN status [GPIO%d: %d]\n", __func__,
			hs_cfg.fsa_switch_gpio[FSA4476_ENN],
			gpio_get_value(hs_cfg.fsa_switch_gpio[FSA4476_ENN]));
		pr_info("%s headset USB FSA4476 EN1 status [GPIO%d: %d]\n", __func__,
			hs_cfg.fsa_switch_gpio[FSA4476_EN1],
			gpio_get_value(hs_cfg.fsa_switch_gpio[FSA4476_EN1]));
		pr_info("%s headset USB FSA4476 EN2 status [GPIO%d: %d]\n", __func__,
			hs_cfg.fsa_switch_gpio[FSA4476_EN2],
			gpio_get_value(hs_cfg.fsa_switch_gpio[FSA4476_EN2]));
	} else if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA3030) {
		pr_info("%s headset USB FSA3030 Sel0 status [GPIO%d: %d]\n", __func__,
			hs_cfg.fsa_switch_gpio[0],
			gpio_get_value(hs_cfg.fsa_switch_gpio[0]));
		pr_info("%s headset USB FSA3030 Sel1 status [GPIO%d: %d]\n", __func__,
			hs_cfg.fsa_switch_gpio[1],
			gpio_get_value(hs_cfg.fsa_switch_gpio[1]));
		pr_info("%s headset USB FSA3030 Sel2 status [GPIO%d: %d]\n", __func__,
			hs_cfg.fsa_switch_gpio[2],
			gpio_get_value(hs_cfg.fsa_switch_gpio[2]));
	}
	return 0;
}
				...
				const char *buf, size_t count)
{
	...
	if (strncmp(buf, "s3_p", count - 1) == 0) {
		if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA4476) {
			gpio_set_value(hs_cfg.fsa_switch_gpio[FSA4476_EN2], 1);
		} else if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA3030) {
		...
		if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA4476) {
			gpio_set_value(hs_cfg.fsa_switch_gpio[FSA4476_EN2], 0);
		} else if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA3030) {
		...
		if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA4476) {
			gpio_set_value(hs_cfg.fsa_switch_gpio[FSA4476_ENN], 0);
			gpio_set_value(hs_cfg.fsa_switch_gpio[FSA4476_EN1], 0);
		} else if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA3030) {
			gpio_set_value(hs_cfg.fsa_switch_gpio[0], 0);
			gpio_set_value(hs_cfg.fsa_switch_gpio[1], 1);
		}
	} else if (strncmp(buf, "fsa_aud", count - 1) == 0) {
		if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA4476) {
			gpio_set_value(hs_cfg.fsa_switch_gpio[FSA4476_ENN], 0);
			gpio_set_value(hs_cfg.fsa_switch_gpio[FSA4476_EN1], 1);
		} else if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA3030) {
			gpio_set_value(hs_cfg.fsa_switch_gpio[0], 1);
			gpio_set_value(hs_cfg.fsa_switch_gpio[1], 0);
		}
	} else {
...
/* HTC_AUD_START */
	if (test_and_clear_bit(WCD_MBHC_ANC0_OFF_ACK,
				&mbhc->hph_anc_state)) {
		usleep_range(HTC_ANC_SWITCH5_DEBOUNCE_TIME, HTC_ANC_SWITCH5_DEBOUNCE_TIME);// Only add debounce for ANC temp disable
		pr_debug("%s: HPHL ANC clear flag and enable ANC_EN\n",
				__func__);
		snd_soc_update_bits(mbhc->codec, WCD934X_CDC_RX1_RX_PATH_CFG0,
				    0x10, 0x10);
	}
	if (test_and_clear_bit(WCD_MBHC_ANC1_OFF_ACK,
		...
		pr_debug("%s: HPHR ANC clear flag and enable ANC_EN\n",
				__func__);
		snd_soc_update_bits(mbhc->codec, WCD934X_CDC_RX2_RX_PATH_CFG0,
				    0x10, 0x10);
	}
/* HTC_AUD_END */
...
static int htc_headset_irq_handler(struct wcd_mbhc *mbhc){
	...
	if (mbhc->current_plug == MBHC_PLUG_TYPE_NONE && insert) {
		...
		if (gpio_get_value(hs_cfg.id_gpio[TYPEC_POSITION])) {
			...
			if (mbhc->mbhc_fn && mbhc->mbhc_fn->htc_button_detection)
				mbhc->mbhc_fn->htc_button_detection(mbhc, true);
		} else {
			ret = -1;
		...
		mbhc->current_plug = MBHC_PLUG_TYPE_NONE;
		if (mbhc->mbhc_fn && mbhc->mbhc_fn->htc_button_detection)
			mbhc->mbhc_fn->htc_button_detection(mbhc, false);
	} else {
...
void set_fsa_switch_accessory(struct wcd_mbhc *mbhc, bool en)
{
	...
	pr_warn("%s: fsa switch mode: %s\n", __func__, en ? "accessory" : "usb");
	...
	if (en) {
		/* accessory mode */
		if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA4476) {
			gpio_set_value(hs_cfg.fsa_switch_gpio[FSA4476_ENN], 0); // ENN
			gpio_set_value(hs_cfg.fsa_switch_gpio[FSA4476_EN1], 1); // EN1
		} else if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA3030) {
			gpio_set_value(hs_cfg.fsa_switch_gpio[0], 1); // SEL0
			gpio_set_value(hs_cfg.fsa_switch_gpio[1], 0); // SEL1
		}
	} else{
		/* USB mode */
		if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA4476) {
			gpio_set_value(hs_cfg.fsa_switch_gpio[FSA4476_ENN], 0); // ENN
			gpio_set_value(hs_cfg.fsa_switch_gpio[FSA4476_EN1], 0); // EN1
		} else if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA3030) {
			gpio_set_value(hs_cfg.fsa_switch_gpio[0], 0); // SEL0
			gpio_set_value(hs_cfg.fsa_switch_gpio[1], 1); // SEL1
		}
	}
}

#define ANC_Headset_Detection_Mask 4
void set_htc_ext_micbias(struct wcd_mbhc *mbhc, bool enable)
{
	struct htc_headset_config hs_cfg;
	if(!mbhc) {
		pr_err("%s: mbhc is NULL\n", __func__);
	...
	hs_cfg = mbhc->mbhc_cfg->htc_headset_cfg;
	hs_cfg.set_ext_micbias(enable, ANC_Headset_Detection_Mask);
}
...
/* HTC_AUD_START - AS HS */
		...
		if (hs_cfg.htc_headset_init) {
			...
			if (insert) {
				...
				set_fsa_switch_accessory(mbhc, true);
				gpio_set_value(hs_cfg.switch_gpio[HEADSET_S5], 1);
				set_htc_ext_micbias(mbhc, true);
				if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA4476) {
					gpio_set_value(hs_cfg.fsa_switch_gpio[FSA4476_EN2], 0);
				} else if (hs_cfg.fsa_switch_type == FSA_SWITCH_FSA3030) {
					gpio_set_value(hs_cfg.switch_gpio[HEADSET_S3_0], 0);
				...
				set_htc_ext_micbias(mbhc, false);
				wcd_mbhc_clr_and_turnon_hph_padac(mbhc);
				...
				set_fsa_switch_accessory(mbhc, false);
			...
			/* HTC_AUD - report unsupport if it's non HTC analog adapter */
			if (mbhc->current_plug != MBHC_PLUG_TYPE_25MM_HEADSET && mbhc->current_plug != MBHC_PLUG_TYPE_AS_HEADSET) {
				if (insert) {
					htc_misc_extcon_set_state(EXTCON_AUDIO_UNSUPPORTED, 1);
					pr_info("%s: Reporting insertion unsupported device \n", __func__);
				} else {
					htc_misc_extcon_set_state(EXTCON_AUDIO_UNSUPPORTED, 0);
...
static irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	if (wcd_swch_level_remove(mbhc)) {
		pr_info("%s: Switch level is low ", __func__); // HTC_AUD for debug
		goto done;
	}
...
//HTC_AUD_END
	pr_info("%s: leave ret %d\n", __func__, ret); // HTC_AUD for debug
	return ret;
