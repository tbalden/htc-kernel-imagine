 ...
 */
#define DEBUG // HTC_AUD for debug
					   ...
					   int *spl_hs_cnt)
{
	...
	if (spl_hs)
		pr_info("%s: Detected special HS (%d)\n", __func__, spl_hs); // HTC_AUD for debug
...

{
	...
	pr_info("%s: plug type is %d found (adc=%d)\n", __func__, plug_type, adc_result); // HTC_AUD for debug
...
#else
        if (mbhc->mbhc_cfg->htc_headset_cfg.swap_detect) {
                do {
                        cross_conn = wcd_check_cross_conn(mbhc);
                        try++;
...
Set micbias_enable to false if type is MBHC_PLUG_TYPE_HIGH_HPH to fix issue.
*/
                        mbhc->micbias_enable = false;
/* HTC_AUD_END */
...
static void htc_button_detection(struct wcd_mbhc *mbhc, bool enable)
{
        ...
        if (mbhc->mbhc_cb->mbhc_micbias_control) {
                wcd_mbhc_adc_update_fsm_source(mbhc, mbhc->current_plug);
        //} else {
...
// HTC_AUD_START
	.htc_button_detection = htc_button_detection,
// HTC_AUD_END
};
