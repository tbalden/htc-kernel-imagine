...
/* HTC_AUD_START: Add for ALT */
		pTAS2557->mTempRear %= TEMPERATURE_AVG_COUNT;
		pTAS2557->mTempBuf[pTAS2557->mTempRear] = (nTemp >> 23);

		pTAS2557->mnSenseTemp++;
		pTAS2557->mTempRear++;

		if (pTAS2557->mnSenseTemp >= TEMPERATURE_AVG_COUNT) {
			int i =0, avgTemp = 0;
			for (i = 0; i < TEMPERATURE_AVG_COUNT; i++)
				avgTemp += pTAS2557->mTempBuf[i];

			avgTemp /= TEMPERATURE_AVG_COUNT;

			dev_dbg(pTAS2557->dev, "avg Die temp %d\n", avgTemp);

			if (avgTemp < GAIN_PROTECT_THREASHOLD) {
				...
				if (pTAS2557->mnDevCurrentGain != pTAS2557->mnDevGain\
					 && avgTemp > GAIN_RECOVER_THREASHOLD) {
			...
			pTAS2557->mnSenseTemp = TEMPERATURE_AVG_COUNT - 1;
		}
/* HTC_AUD_END */
...
/* HTC_AUD_START */
	...
	case TAS2557_PG_VERSION_2P1:
		...
		pTAS2557->dev, GFP_KERNEL, pTAS2557, tas2557_fw_ready);
		dev_info(&pClient->dev, "%s loading %s\n", __func__, TAS2557_PG21_FW_NAME);
		break;
