...
/* HTC_AUD_START HPKB:332 */
struct wakeup_source compr_lpa_q6_cb_wakelock;
/* HTC_AUD_END */
...
/* HTC_AUD_START HPKB:332 */
		__pm_wakeup_event(&compr_lpa_q6_cb_wakelock, jiffies_to_msecs(5 * HZ));
...
/* HTC_AUD_END */
			prtd->last_buffer = 0;
/* HTC_AUD_START HPKB:332 - Must release wakelock at drain/eos state due to upper layer won't handle wakelock*/
#if 0
		} else
			msm_compr_send_buffer(prtd);
#else
...
/* HTC_AUD_END */
		...
		/* atomic_set(&prtd->eos, 0); HTC_AUD HPKB:332 */
...
static int msm_compr_playback_open(struct snd_compr_stream *cstream)
{
	...
	atomic_set(&prtd->drain_done_pendding, 0); /* HTC_AUDIO HPKB:332 */
...
/* HTC_AUD_START HPKB:2082 - Add tinymix control for q6asm effect */
...
/* HTC_AUD_END */
	...
	case SNDRV_PCM_TRIGGER_STOP:
		...
		if (atomic_read(&prtd->drain)) {
			...
			prtd->drain_ready = 1;
			wake_up(&prtd->drain_wait);
			/* atomic_set(&prtd->drain, 0); HTC_AUD HPKB:332 */
		}
		prtd->last_buffer = 0;
		prtd->cmd_ack = 0;
/* HTC_AUD_START HPKB:332 */
...
/* HTC_AUD_END */
	...
	case SND_COMPR_TRIGGER_NEXT_TRACK:
		...
		rc = q6asm_stream_open_write_v4(prtd->audio_client,
				prtd->codec, prtd->bits_per_sample, /* HTC_AUD HPKB:210 */
				stream_id,
				prtd->gapless_state.use_dsp_gapless_mode);
		if (rc < 0) {
...
{
/* HTC_AUD_START HPKB:332 */
	wakeup_source_init(&compr_lpa_q6_cb_wakelock, "compr_lpa_q6_cb");
/* HTC_AUD_END */
