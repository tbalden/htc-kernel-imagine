...
#include <asoc/htc_acoustic_alsa.h>
/* HTC_AUD_END */
...
static struct dev_config mi2s_rx_cfg[] = {
	...
	[QUAT_MI2S] = {SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S24_LE, 2}, /* HTC_AUDIO HPKB:28982 QUAT_MI2S 24bit */
};
...
static struct dev_config mi2s_tx_cfg[] = {
	...
	[QUAT_MI2S] = {SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S24_LE, 1}, /* HTC_AUDIO HPKB:28982 QUAT_MI2S 24bit */
};
...
static const char *const mi2s_ch_text[] = {"One", "Two", "Three", "Four",
					   "Five", "Six", "Seven",
					   "Eight"};
static const char *const mi2s_format_text[] = {"S16_LE", "S24_LE"}; /* HTC_AUD HPKB:28982 - Add QUAT_MI2S_RX Format control */
...
/* HTC_AUD_START HPKB:28982 - mclk patch for tas2557 config */
...
/* HTC_AUD_END HPKB:28982 - mclk patch for tas2557 config */
...
/* HTC_AUD_END - HTC Effect {HPKB:2082} */

static void set_htc_ext_micbias(bool enable, short mask)
{
	mutex_lock(&ext_micbias_mutex);
	if (enable) {
		ext_micbias_count |= mask;
		if (ext_micbias_count != 0 && gpio_get_value(wcd_mbhc_cfg.htc_headset_cfg.ext_micbias) != 1) {
...
#define ANCLeft_Headset_Mic_Mask 0x1
#define ANCRight_Headset_Mic_Mask 0x2
static int htc_typec_hs_lr_event(struct snd_soc_dapm_widget *w,
                                 struct snd_kcontrol *k, int event)
{
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		if (!strcmp(w->name, "ANCLeft Headset Mic")) {
			set_htc_ext_micbias(true, ANCLeft_Headset_Mic_Mask);
		}
		else if (!strcmp(w->name, "ANCRight Headset Mic")) {
			set_htc_ext_micbias(true, ANCRight_Headset_Mic_Mask);
		}
		else {
		...
		if (!strcmp(w->name, "ANCLeft Headset Mic")) {
			set_htc_ext_micbias(false, ANCLeft_Headset_Mic_Mask);
		}
		else if (!strcmp(w->name, "ANCRight Headset Mic")) {
			set_htc_ext_micbias(false, ANCRight_Headset_Mic_Mask);
		}
		else {
			pr_err("%s: wrong widget in ON event", __func__);
			return -EINVAL;
...
/* HTC_AUD_START HPKB:3862 - add delay to avoid dmic enable pop sound */
...
/* HTC_AUD_END HPKB:3862 - add delay to avoid dmic enable pop sound */
...
/* HTC_AUD_START HPKB:28982 - Add validation check to prevent KP*/
...
clk_off:
	if (ret < 0) {
		msm_mi2s_set_mclk(substream, false); /* HTC_AUD HPKB:28982 - mclk patch for tas2557 config */
...
/* HTC_AUD_END */
	...
	if (--mi2s_intf_conf[index].ref_cnt == 0) {
		ret = msm_mi2s_set_mclk(substream, false); /* HTC_AUD HPKB:28982 - mclk patch for tas2557 config */
...
static int headset_dt_parser(struct platform_device *pdev, struct wcd_mbhc_config *wcd_mbhc_cfg)
{
	...
	const char *fsa_switch_type_name = "htc,usb_hph_switch";
	const char *fsa_switch_gpio_name[FSA_SWITCH_GPIO_COUNT] = {
			"htc,usb_hph_switch_gpio0",
			"htc,usb_hph_switch_gpio1",
			"htc,usb_hph_switch_gpio2"};
	const unsigned long fsa3030_switch_gpio_init_state[FSA_SWITCH_GPIO_COUNT] = {
			GPIOF_OUT_INIT_LOW,  // SEL0
			GPIOF_OUT_INIT_HIGH, // SEL1
			GPIOF_OUT_INIT_LOW}; // SEL2
	const unsigned long fsa4476_switch_gpio_init_state[FSA_SWITCH_GPIO_COUNT] = {
			GPIOF_OUT_INIT_LOW,  // ENN
			GPIOF_OUT_INIT_LOW,  // EN1
			GPIOF_OUT_INIT_LOW}; // EN2
	const unsigned long *fsa_switch_gpio_init_state = NULL;
	const char *mbhc_swap_detect = "htc,mbhc-swap-detect";
	const char *name;
	int ret = 0;
	...
	if (IS_ERR_VALUE((unsigned long)id_gpio_count) && IS_ERR_VALUE((unsigned long)switch_gpio_count)) {
...
#if 0
	/* when fsa4476 is used, we do not need ext 2 PINs for S3 */
	...
	ret = of_property_read_string(pdev->dev.of_node,
					fsa_switch_type_name, &name);
	if (ret) {
		pr_warn("%s: of read string %s error %d\n",
			__func__, fsa_switch_type_name, ret);
		name = NULL;
	}
	if (name != NULL) {
		if (!strncmp(name, "fsa3030", strlen("fsa3030"))) {
			wcd_mbhc_cfg->htc_headset_cfg.fsa_switch_type = FSA_SWITCH_FSA3030;
			fsa_switch_gpio_init_state = fsa3030_switch_gpio_init_state;
		} else if (!strncmp(name, "fsa4476", strlen("fsa4476"))) {
			wcd_mbhc_cfg->htc_headset_cfg.fsa_switch_type = FSA_SWITCH_FSA4476;
			fsa_switch_gpio_init_state = fsa4476_switch_gpio_init_state;
		} else {
			wcd_mbhc_cfg->htc_headset_cfg.fsa_switch_type = FSA_SWITCH_NONE;
			fsa_switch_gpio_init_state = NULL;
		}
	}
	...
	for (i = 0; i < switch_gpio_count; i++) {
		...
		if (gpio_is_valid(wcd_mbhc_cfg->htc_headset_cfg.switch_gpio[i])) {
			...
			if (i == 3) { /* hpmic_agnd_flip_en_s1: O(H) */
	...
	/* USB/Audio switch */
	if (wcd_mbhc_cfg->htc_headset_cfg.fsa_switch_type != FSA_SWITCH_NONE) {
		for (i = 0; i < FSA_SWITCH_GPIO_COUNT; i++) {
			wcd_mbhc_cfg->htc_headset_cfg.fsa_switch_gpio[i] = of_get_named_gpio(pdev->dev.of_node,
										fsa_switch_gpio_name[i], 0);
			if (gpio_is_valid(wcd_mbhc_cfg->htc_headset_cfg.fsa_switch_gpio[i])) {
				pr_info("%s: gpio %s parse success gpio no %d, init %lu\n", __func__,
					fsa_switch_gpio_name[i],
					wcd_mbhc_cfg->htc_headset_cfg.fsa_switch_gpio[i],
					fsa_switch_gpio_init_state[i]);
				gpio_free(wcd_mbhc_cfg->htc_headset_cfg.fsa_switch_gpio[i]);
				ret = gpio_request_one(wcd_mbhc_cfg->htc_headset_cfg.fsa_switch_gpio[i],
					fsa_switch_gpio_init_state[i], fsa_switch_gpio_name[i]);
				if (ret) {
					pr_err("%s: gpio %s request failed with err %d\n", __func__,
						fsa_switch_gpio_name[i], ret);
					return ret;
				}
			} else {
				pr_err("%s: gpio %s no %d parse fail\n", __func__,
						fsa_switch_gpio_name[i], wcd_mbhc_cfg->htc_headset_cfg.fsa_switch_gpio[i]);
				return -EINVAL;
			}
		}
	}
	...
	ret = of_property_read_u32(pdev->dev.of_node, mbhc_swap_detect, &swap_detect);
	if (ret) {
		pr_err("%s: missing mbhc-swap-detect in dt node\n", __func__);
		swap_detect = 0;
	} else {
		pr_info("%s: mbhc_swap_detect %d\n", __func__, swap_detect);
	}
	wcd_mbhc_cfg->htc_headset_cfg.swap_detect = swap_detect;
...
static int sdm845_setparam(htc_adsp_params_ioctl_t *ctrl)
{
	int ret = 0;
	int type = ctrl->type;
...
static int sdm845_get_headsetType(void)
{		
	int ret = 0;
	struct snd_soc_codec *codec;

	list_for_each_entry(codec, &snd_soc_card_tavil_msm.codec_dev_list, card_list) {
		pr_debug("%s: sound card codec name %s\n", __func__, dev_name(codec->dev));
		if (!strncmp(dev_name(codec->dev), "tavil_codec", sizeof("tavil_codec")))
...
static struct acoustic_ops acoustic = {
	.msm_setparam = sdm845_setparam,
	.get_headsetType = sdm845_get_headsetType,
	.set_q6_effect = htc_effect_by_adm_open,
};
/* HTC_AUD_END - AS HS {HPKB:7329} */
...
/* ext micbias function */
		wcd_mbhc_cfg.htc_headset_cfg.set_ext_micbias = set_htc_ext_micbias;
	}
/* ext micbias mutex */
