...
/* HTC_AUD_START */
		if (payload[0] == 0) {
			pr_err("%s:PL#0[0x%x] dir=0x%x s_id=0x%x\n",
			       __func__, payload[0], dir, asm_token._token.session_id);
#ifdef CONFIG_HTC_DEBUG_DSP
			panic("%s: Payload is zero", __func__);
#else
			pr_err("%s: [op:%x] Payload is zero\n", __func__, data->opcode);
#endif
		}
/* HTC_AUD_END */
...
/* HTC_AUD_END */
	...
	if (atomic_read(&ac->cmd_state) > 0) {
		...
		rc = adsp_err_get_lnx_err_code(
				atomic_read(&ac->cmd_state));
/* HTC_AUD_START HPKB:302 - restart adsp if asm close failed and open failed(EALREADY) too at next time */
...
/* HTC_AUD_START */
	...
	pr_err("%s: buf_add 0x%llx, bufsz: %d\n", __func__,
		buf_add, bufsz);
#else
	pr_debug("%s: buf_add 0x%llx, bufsz: %d\n", __func__,
		buf_add, bufsz);
#endif
#endif
/* HTC_AUD_END */
...
EXPORT_SYMBOL(htc_set_asm_effect);
/* HTC_AUD_END */
